...
test(
  ...
  async () => {
    ...
    try {
      ...
      assert.equal(attainment.attainmentCapacityMilliseconds, 456 * minute);
      assert.equal(attainment.creditedAbnormalMilliseconds, 40 * minute);
      assert.equal(attainment.attainmentBasisPoints, 10044);
    } finally {
