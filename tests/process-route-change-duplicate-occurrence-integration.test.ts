...
test(
  ...
  async () => {
    ...
    try {
      ...
      assert.equal(progressedObligation.systemCoveredQty, 0);
      assert.equal(progressedObligation.reportedQty, 0);
      assert.equal(progressedObligation.fulfillmentMode, 'ACTUAL');
      assert.equal(progressedObligation.status, 'ACTIVE');
      assert.equal(progressedObligation.coverage?.policy, 'FULL_WORK_ORDER_REQUIRED');
      assert.equal(progressedObligation.coverage?.routeTargetQty, 10);
      assert.equal(progressedObligation.coverage?.systemCoveredQty, 0);
      assert.equal(progressedObligation.coverage?.actualRequiredQty, 10);
