...
- 进度：原生 App 已接入后端真实登录、工单资料库、工单管理、资料管理、连接器参数、系统设置、日志、回收站、快照、诊断信息和平台 adapter 闭环
...
- 功能：
  ...
  - 资源文件 PDF/图片选择、multipart 上传、预览、下载、打开、复制链接、软删除、恢复接口、文件信息编辑、分类移动和跨工单 ID 移动
  - 连接器参数查询、筛选、新增、编辑、删除、恢复、复制整行参数、批量标记、批量删除、CSV 粘贴导入预览、CSV/XLS/XLSX 文件导入预览、确认导入、导入批次回滚、CSV 导出和原始附件上传/下载/删除
  - 平台 adapter：文件选择、拍照/相册入口、下载打开、剪贴板复制和多处语音输入兜底
