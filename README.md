...
- 当前生产版本：v1.29.0（revision `a86c228e83fcba178c161579f0879e14bf6e43d6`）
...
- 账号登录、修改密码、账号管理
- 生产员工账号扫码报工：沿用现有工单二维码，员工使用员工编号和临时密码 `123456` 登录；该账号仅有扫码报工权限，不进入后台工作台
...
- Windows 微盘导入助手：网页创建 10 分钟短期任务，支持普通用户 HKCU 协议自注册 / 自修复、助手实例幂等任务码、脱敏诊断、真实文件拖拽、文件粘贴、下载目录监控、SHA-256 防重、新版本与失败重试
- 图纸资料库首页：按客户 -> 规格 -> 图纸资料管理长期图纸文件，不保存周计划状态字段
- 问题管理关联工单支持全部未删除工单的服务端搜索与分页；缺失工单可随问题原子创建为“待补资料”工单，图纸、工序和工时后续维护
...
将发布提交推送到 GitHub `main`，再从该提交创建并推送 `v1.33.0` 标签后，工作流会构建：

```text
ghcr.io/gy3117577403-ai/hongmeng:v1.33.0
```

Sealos App Deploy 端口填 `3000`。`v1.33.0` 是本次待验收候选镜像；当前生产实际版本必须在切换前现场核验，完成生产发布前不要把候选版本写成已上线。`latest` 仅用于跟随最新构建，生产切换应固定到已验收的镜像 digest。
