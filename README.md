...
- 功能：
  ...
  - 连接器参数查询、筛选、新增、编辑、删除、恢复、复制整行参数、批量标记、批量删除、CSV 粘贴导入预览、CSV/XLS/XLSX 文件导入预览、确认导入、大批量导入确认、导入批次回滚确认口令、CSV 导出和原始附件上传/下载/删除
  - 平台 adapter：文件选择、现场照片入口、照片预览确认、native 下载短期 ticket、下载打开、剪贴板复制和多处语音/手动输入兜底
  - 设置页：生产稳定中心、系统状态、账号管理、账号启停 / 重置密码二次确认和 Native API 确认口令、修改密码、操作日志、回收站、数据快照和诊断信息
- 验证：
  - `npm run build` 通过
  - `npm run smoke` 通过
