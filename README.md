...
将发布提交推送到 GitHub `main`，再从该提交创建并推送 `v1.32.3` 标签后，工作流会构建：

```text
ghcr.io/gy3117577403-ai/hongmeng:v1.32.3
```

Sealos App Deploy 端口填 `3000`。`v1.32.3` 是本次待验收候选镜像；当前生产实际版本必须在切换前现场核验，完成生产发布前不要把候选版本写成已上线。`latest` 仅用于跟随最新构建，生产切换应固定到已验收的镜像 digest。
