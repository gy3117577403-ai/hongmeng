...
function asciiFilename(filename: string): string {
  return filename.replace(/[^\x20-\x7E]/g, '_').replace(/["\\]/g, '_') || 'change-attachment';
}
...
export async function GET(_req: Request, { params }: { params: { attachmentId: string } }) {
  try {
    ...
    console.error('change attachment content failed', error);
    return NextResponse.json({ ok: false, error: '附件读取失败' }, { status: 500 });
  }
}
