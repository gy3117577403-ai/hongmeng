...
export async function GET(req: NextRequest) {
  try {
    ...
    const rows = await prisma.$queryRaw<Array<{ code: string; customer: string; model: string; issued: string | null; due: string | null; status: string; warningDays: number; taskType: string; week: string | null; quantity: number | null; completedQuantity: number }>>(Prisma.sql`SELECT t.task_type AS "taskType", to_char(t.plan_week_start_date,'YYYY-MM-DD') AS week, t.sample_quantity AS quantity, t.completed_quantity AS "completedQuantity", t.code, t.customer_name_snapshot AS customer, t.specification_snapshot AS model, to_char(t.issued_date,'YYYY-MM-DD') AS issued, to_char(t.due_date,'YYYY-MM-DD') AS due, t.status, t.warning_days AS "warningDays" FROM sample_tasks t WHERE ${q.base} AND (${q.views[q.view]}) ORDER BY ${q.order}`);
    ...
    const data = [['序号', '任务编号', '客户', '产品型号', '样品类型', '计划周', '计划数量', '完成数量', '计划下达日期', '计划出货日期', '状态', '提前预警天数', '交期提示'], ...rows.map((r,i) => [i+1, r.code, r.customer, r.model, r.taskType === 'REPEAT' ? '老产品制作' : '新品试制', r.week || '待排期', r.quantity, r.completedQuantity, r.issued || '未记录', r.due, r.taskType === 'REPEAT' && r.status === 'IN_PROGRESS' ? '制作中' : status[r.status] || r.status, r.warningDays, sampleWarning({ status: r.status, dueDate: r.due, warningDays: r.warningDays }).label])];
