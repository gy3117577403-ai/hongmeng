...
import { samplePlanTime, sampleHours } from '@/lib/sample-plan-time';
...
export async function GET(req: NextRequest) {
  try {
    ...
    const rows = await prisma.$queryRaw<Array<{ code: string; customer: string; model: string; issued: string | null; due: string | null; status: string; warningDays: number; taskType: string; week: string | null; plannedCompletion: string | null; quantity: number | null; completedQuantity: number;completedQuantityKnown:boolean;unitPlannedMilliseconds:number|null;sourceOrderNo:string|null;sourceOrderLine:string|null;planRemark:string|null }>>(Prisma.sql`SELECT t.completed_quantity_known AS "completedQuantityKnown", t.unit_planned_milliseconds AS "unitPlannedMilliseconds", t.source_order_no AS "sourceOrderNo", t.source_order_line AS "sourceOrderLine", t.plan_remark AS "planRemark", to_char(t.planned_completion_date,'YYYY-MM-DD') AS "plannedCompletion", t.task_type AS "taskType", to_char(t.plan_week_start_date,'YYYY-MM-DD') AS week, t.sample_quantity AS quantity, t.completed_quantity AS "completedQuantity", t.code, t.customer_name_snapshot AS customer, t.specification_snapshot AS model, to_char(t.issued_date,'YYYY-MM-DD') AS issued, to_char(t.due_date,'YYYY-MM-DD') AS due, t.status, t.warning_days AS "warningDays" FROM sample_tasks t WHERE ${q.base} AND (${q.views[q.view]}) ORDER BY ${q.order}`);
