...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ ok: true, data: await loadFinishedGoods({ date: p.get('date') || undefined, view: p.get('view') || undefined, filter: p.get('filter') || undefined, q: p.get('q') || undefined, batchId: p.get('batchId') || undefined, page: Number(p.get('page')) || 1, pageSize: Number(p.get('pageSize')) || 24 }) });
