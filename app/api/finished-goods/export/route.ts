...
export async function GET(request: NextRequest) {
  try {
    ...
    const query = { date: p.get('date') || undefined, dateTo: p.get('dateTo') || undefined, scope: p.get('scope') || undefined, view: p.get('view') || 'history', filter: p.get('filter') || undefined, q: p.get('q') || undefined, batchId: p.get('batchId') || undefined, pageSize: 100 };
    ...
    const headers = ['状态','工单号','客户','产品','规格','单位','待入库','在库数量','已出数量','累计入库','可用','占用','留库','隔离','本次出库','已退回','出库记录号','日出货批次','实际出库时间','出货方式','外部单号','备注','库位','留库原因','留库到期','入库时间','历史结清时间','历史结清数量','本次入库'];
    ...
    for (const r of rows) sheet.addRow([states[r.status] || r.status,r.workOrderCode,r.customerName || '公共备货',r.productName,r.specification,r.unit,r.pending,r.onHand,r.legacyClosedAt ? null : r.shippedQuantity,r.legacyClosedAt ? null : r.receivedQuantity,r.available,r.reserved,r.held,r.blocked,r.status === 'shipped' ? r.quantity : 0,r.returned,r.shipmentNumber || '',r.batchNumber,time(r.status==='shipped' ? r.shippedAt : r.lastShippedAt),({COURIER:'快递 / 物流',PICKUP:'自提',DELIVERY:'送货'} as Record<string,string>)[r.method] || r.method,r.externalReference,r.shipmentNote || r.note,r.location,r.holdReason,r.holdDueDate,time(r.status==='received' ? r.receivedAt : r.lastReceivedAt),time(r.legacyClosedAt),r.legacyQuantity,r.status === 'received' ? r.quantity : 0]);
