  ...
  try {
    ...
    if (q.has("summary")) {
      ...
      const [supervisor, quality, draft, purchasing] = await Promise.all([
        settings?.supervisorIds.includes(actor.id) ? prisma.qfPackage.count({ where: { status: "SUPERVISOR", submittedById: { not: actor.id }, libraryItem: { deletedAt: null } } }) : 0,
        settings?.qualityIds.includes(actor.id) ? prisma.qfPackage.count({ where: { status: "QUALITY", submittedById: { not: actor.id }, supervisorId: { not: actor.id }, libraryItem: { deletedAt: null } } }) : 0,
        prisma.qfPackage.count({ where: { status: { in: ["DRAFT", "RETURNED"] }, libraryItem: { deletedAt: null } } }),
