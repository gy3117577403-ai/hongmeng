...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const assigned = await prisma.internalQualityRiskReport.findUnique({ where: { id: params.id }, select: { ownerUserId: true, createdById: true, workflowVersion: true } });
    if (assigned?.workflowVersion && assigned.workflowVersion >= 3) {
      if (body.status !== 'SUBMITTED') return NextResponse.json({ ok: false, error: '请在阶段处理或独立品质确认页面操作' }, { status: 409 });
      const report = await prisma.$transaction(tx => actOnQualityWorkflow(tx, params.id, expectedInternalQualityRiskVersion(body.expectedVersion), 'SUBMIT', {}, actor));
