...
export async function GET(_req:NextRequest,{params}:{params:{id:string}}){try{const user=await requireUser(); const f=await prisma.resourceFile.findFirst({where:{id:params.id,deletedAt:null,status:'uploaded'}}); if(!f)return NextResponse.json({message:'文件不存在'},{status:404}); await logOp({userId:user.id,action:'download',targetType:'resource_file',targetId:f.id}); return NextResponse.redirect(await signedUrl({key:f.objectKey,filename:safeDisplayFilename(f),disposition:'attachment',contentType:f.mimeType}))}catch(e){if(e instanceof UnauthorizedError)return unauthorized(); console.error(e); return NextResponse.json({message:'文件下载失败'},{status:500})}}
