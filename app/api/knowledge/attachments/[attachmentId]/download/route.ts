...
export async function GET(_req: Request, { params }: { params: { attachmentId: string } }) {
  try {
    ...
    await logOp({ userId: user.id, action: 'download_knowledge_attachment', targetType: 'knowledge_attachment', targetId: attachment.id, detail: { articleId: attachment.articleId } });
    ...
    console.error('knowledge attachment download failed', error);
    return NextResponse.json({ ok: false, error: '知识附件下载失败' }, { status: 500 });
  }
}
