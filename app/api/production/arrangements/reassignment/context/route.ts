...
import { getProductionArrangementReassignmentContext } from '@/lib/daily-plan-service';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await getProductionArrangementReassignmentContext({
      actorUserId: user.id,
      workOrderIds: workOrderIds(request),
      planId: params.get('planId'),
      sourceEmployeeId: params.get('sourceEmployeeId'),
    });
    return dailyPlanSuccess(data);
  } catch (error) {
    return dailyPlanError(error, 'load production arrangement reassignment context');
  }
}
