...
export async function GET(req: NextRequest) {
  try {
    ...
    const result = await prisma.$transaction(async tx => {
      const [total, pending] = await Promise.all([tx.connectorParameterConflict.count({ where }), tx.connectorParameterConflict.count({ where: { status: 'PENDING' } })]);
      const totalPages = Math.max(1, Math.ceil(total / 20));
      const currentPage = Math.min(Math.floor(page), totalPages);
      const ids = await tx.connectorParameterConflict.findMany({ where, orderBy: [{ createdAt: 'desc' }, { id: 'asc' }], take: 20, skip: (currentPage - 1) * 20, select: { id: true } });
      const items = await Promise.all(ids.map(row => connectorConflictDetail(tx, row.id)));
      return { items, total, pending, page: currentPage, totalPages };
