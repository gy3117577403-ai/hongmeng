...
export async function POST(req: NextRequest) {
  try {
    ...
    const batch = await prisma.connectorParameterImportBatch.create({
      data: {
        sourceType: String(body.sourceType || 'unknown').slice(0, 40),
    ...
    await prisma.connectorParameterImportBatch.update({
      ...
      data: {
        ...
        summaryJson: { ...previewSummary, created, skipped, failed, duplicateSkipped },
      },
    });
    ...
    await logOp({
      ...
      detail: { created, skipped, failed, duplicateSkipped, duplicateStrategy, total: results.length, batchId: batch.id },
    });
    await logOp({
      userId: user.id,
      action: 'create_connector_parameter_import_batch',
    ...
    await snapshotChange({
      ...
      entityId: batch.id,
      action: 'create_connector_parameter_import_batch',
      after: { id: batch.id, sourceType: batch.sourceType, fileName: batch.fileName, insertedCount: created, totalRows: rows.length },
      changedBy: userName,
    });
    ...
    return NextResponse.json({
      ...
      batch: { id: batch.id, insertedCount: created },
    });
  } catch (e) {
