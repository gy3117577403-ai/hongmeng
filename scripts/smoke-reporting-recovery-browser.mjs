...
try {
  ...
  const code = `async page => {
    ...
    await page.getByRole('heading',{name:'核对实际完成的整套数量'}).waitFor();
    check(await page.locator('textarea[required],input[type=text][required]').count()===0,'no mandatory free text');
    const submit=page.getByRole('button',{name:'确认并完成原报工',exact:true});
    check(await submit.isDisabled(),'ambiguous action mapping requires explicit selection');
    await page.getByLabel('形成完整产品（套）',{exact:true}).fill('40');
    await page.getByLabel('已核对动作数量与实际整套数量',{exact:true}).check();
    ...
    await submit.click();
    await page.getByRole('heading',{name:'原报工已完成',exact:true}).waitFor({timeout:90000});
    ...
    await page.goto(base+'/workspace/reporting-recovery?id='+id);
    await page.getByRole('heading',{name:'原报工已完成',exact:true}).waitFor();
    check(await page.getByRole('button',{name:'确认并完成原报工',exact:true}).count()===0,'operator sees own receipt without handler controls');
