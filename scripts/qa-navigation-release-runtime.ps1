param(
  ...
  [ValidatePattern('^hongmeng_navigation_v134140_release(?:_[a-z0-9]+)?$')][string]$DatabaseName = 'hongmeng_navigation_v134140_release',
  [ValidatePattern('^hm-navigation-v134140-release-app(?:-[a-z0-9]+)?$')][string]$AppName = 'hm-navigation-v134140-release-app'
)

$ErrorActionPreference = 'Stop'
$taskRoot = [IO.Path]::GetFullPath((Join-Path $PSScriptRoot '..'))
$taskLabel = 'navigation-v134140-release'
$networkName = 'hm-navigation-v134140-release-net'
$postgresName = 'hm-navigation-v134140-release-postgres'
$minioName = 'hm-navigation-v134140-release-minio'
$baseUrl = 'http://127.0.0.1:3111'
$expectedVersion = 'v1.34.140'
$hostEnvPath = Join-Path $taskRoot '.env.navigation-release.local'
$containerEnvPath = Join-Path $taskRoot '.docker/navigation-v134140-release.env'
$evidenceDirectory = Join-Path $taskRoot 'artifacts/navigation-v134140'
...
function Assert-TaskDatabaseTargets {
  ...
  if (!$hostDatabase.IsAbsoluteUri -or !$containerDatabase.IsAbsoluteUri -or
    ...
    $hostDatabase.Host -ne '127.0.0.1' -or $hostDatabase.Port -ne 55440 -or
    ...
    $hostDatabase.UserInfo.Split(':', 2)[0] -ne 'navqa' -or $hostDatabase.UserInfo -ne $containerDatabase.UserInfo) {
...
try {
  if ($Stage -eq 'prepare') {
    $occupiedPorts = @(Get-NetTCPConnection -State Listen -ErrorAction SilentlyContinue | Where-Object {
      $_.LocalPort -in 3111, 55440, 19042, 19043
    })
    ...
    $hostEnvironment = [ordered]@{
      ...
      NAVIGATION_QA_BASE = $baseUrl
      DATABASE_URL = "postgresql://navqa:$databasePassword@127.0.0.1:55440/${databaseName}?schema=public"
      SESSION_SECRET = $sessionSecret
      SEED_ADMIN_USERNAME = 'navbootstrap'
      SEED_ADMIN_PASSWORD = $bootstrapPassword
      S3_ENDPOINT = 'http://127.0.0.1:19042'
      S3_PUBLIC_ENDPOINT = 'http://127.0.0.1:19042'
      S3_REGION = 'auto'
      S3_BUCKET = 'workorder-resources'
      S3_ACCESS_KEY_ID = 'navqaaccess'
      S3_SECRET_ACCESS_KEY = $storageSecret
      S3_FORCE_PATH_STYLE = 'true'
      DAILY_PLAN_ENABLED = 'true'
      NAVIGATION_QA_ALLOW = 'disposable-navigation-runtime'
      NAVIGATION_QA_CHECK_BOOTSTRAP = '1'
      NAVIGATION_QA_PASSWORD = $fixturePassword
      EXPECTED_APP_VERSION = $expectedVersion
      NAVIGATION_QA_EVIDENCE = 'artifacts/navigation-v134140/release-runtime-smoke.json'
    }
    ...
    $containerEnvironment.DATABASE_URL = "postgresql://navqa:$databasePassword@${postgresName}:5432/${databaseName}?schema=public"
    ...
    if ($networkName -in $existingNetworks) {
      ...
      Invoke-TaskDocker @('network', 'create', '--label', "codex.task=$taskLabel", '--subnet', '10.249.140.0/24', $networkName) | Out-Null
    }
    Invoke-TaskDocker @('run', '-d', '--name', $postgresName, '--label', "codex.task=$taskLabel", '--network', $networkName,
      '--tmpfs', '/var/lib/postgresql/data', '-p', '127.0.0.1:55440:5432', '-e', 'POSTGRES_USER=navqa',
    ...
    Invoke-TaskDocker @('run', '-d', '--name', $minioName, '--label', "codex.task=$taskLabel", '--network', $networkName,
      '--tmpfs', '/data', '-p', '127.0.0.1:19042:9000', '-p', '127.0.0.1:19043:9001',
      '-e', 'MINIO_ROOT_USER=navqaaccess', '-e', "MINIO_ROOT_PASSWORD=$storageSecret",
...
'@
    ...
    Invoke-TaskDocker @('exec', $postgresName, 'pg_isready', '-U', 'navqa', '-d', $databaseName) | Out-Null
    ...
    Write-Output "Reserved application: $baseUrl; PostgreSQL: 55440; MinIO: 19042/19043."
  ...
  if ($databasePort.Count -ne 1 -or $databasePort[0] -ne '127.0.0.1:55440') {
    throw 'The host fixture port is not assigned to the dedicated PostgreSQL container.'
  }
  $publicTableCount = Invoke-TaskDocker @('exec', $postgresName, 'psql', '-U', 'navqa', '-d', $databaseName, '-Atc',
  ...
  Invoke-TaskDocker @('run', '-d', '--name', $appName, '--label', "codex.task=$taskLabel", '--network', $networkName,
    '-p', '127.0.0.1:3111:3000', '--env-file', $containerEnvPath, $pinnedImage) | Out-Null
  $ready = $false
  ...
  Invoke-TaskNode @('--import', 'tsx', 'scripts/qa-navigation-seed.ts')
  Invoke-TaskNode @('scripts/smoke-platform-navigation.mjs')
  $migrationCount = Invoke-TaskDocker @('exec', $postgresName, 'psql', '-U', 'navqa', '-d', $databaseName, '-Atc',
