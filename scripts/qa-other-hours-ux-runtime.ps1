...
$taskLabel = 'other-hours-ux-v134151'
$network = 'hm-other-hours-ux-v134151-net'
$postgres = 'hm-other-hours-ux-v134151-postgres'
$minio = 'hm-other-hours-ux-v134151-minio'
$app = 'hm-other-hours-ux-v134151-app'
$database = 'hongmeng_other_hours_ux_v134151'
$envPath = Join-Path $taskRoot '.env.other-hours-ux.local'
$containerEnv = Join-Path $taskRoot '.docker/other-hours-ux.env'
...
try {
  if ($Stage -eq 'prepare') {
    ...
    if (@(Get-NetTCPConnection -State Listen -ErrorAction SilentlyContinue | Where-Object { $_.LocalPort -in 3122,55452,19062,19063 }).Count) { throw 'Dedicated test port is occupied' }
    $existing = @(Docker @('ps','-a','--format','{{.Names}}'))
    if (@($existing | Where-Object { $_ -in $postgres,$minio,$app }).Count) { throw 'Dedicated container name is occupied' }
    New-Item -ItemType Directory -Force .docker,output/playwright/other-hours-ux-v134151 | Out-Null
    $dbPassword = 'OtherDb-' + [guid]::NewGuid().ToString('N')
    $storagePassword = 'OtherS3-' + [guid]::NewGuid().ToString('N')
    $envs = [ordered]@{
      DATABASE_URL = "postgresql://otherqa:${dbPassword}@127.0.0.1:55452/${database}?schema=public"
      SESSION_SECRET = 'OtherSession-' + [guid]::NewGuid().ToString('N')
      APP_BASE_URL = 'http://127.0.0.1:3122'
      S3_ENDPOINT = 'http://127.0.0.1:19062'
      S3_PUBLIC_ENDPOINT = 'http://127.0.0.1:19062'
      S3_REGION = 'auto'
      S3_BUCKET = 'other-hours-ux-qa'
    ...
    Docker @('network','create','--label',"codex.task=$taskLabel",'--subnet','10.249.151.0/24',$network) | Out-Null
    Docker @('run','-d','--name',$postgres,'--label',"codex.task=$taskLabel",'--network',$network,'--tmpfs','/var/lib/postgresql/data','-p','127.0.0.1:55452:5432','-e','POSTGRES_USER=otherqa','-e',"POSTGRES_PASSWORD=$dbPassword",'-e',"POSTGRES_DB=$database",'postgres:16-alpine') | Out-Null
    Docker @('run','-d','--name',$minio,'--label',"codex.task=$taskLabel",'--network',$network,'--tmpfs','/data','-p','127.0.0.1:19062:9000','-p','127.0.0.1:19063:9001','-e','MINIO_ROOT_USER=otherqaaccess','-e',"MINIO_ROOT_PASSWORD=$storagePassword",'minio/minio:latest','server','/data','--console-address',':9001') | Out-Null
    Write-Output 'Created isolated PostgreSQL and MinIO for other-hours v1.34.151. Credentials remain in ignored local files.'
    return
  }
  foreach ($line in Get-Content -LiteralPath $envPath) {
    if ($line -match '^([A-Z][A-Z0-9_]*)=(.*)$') { [Environment]::SetEnvironmentVariable($Matches[1],$Matches[2],'Process') }
  }
  if ([uri]$env:DATABASE_URL -eq $null -or $env:DATABASE_URL -notmatch '@127.0.0.1:55452/hongmeng_other_hours_ux_v134151(_release)?\?') { throw 'Unexpected database target' }
  if ($Stage -eq 'release-prepare') {
    ...
    Docker @('exec',$postgres,'createdb','-U','otherqa','hongmeng_other_hours_ux_v134151_release') | Out-Null
    foreach ($file in @($envPath,$containerEnv)) {
      $contents = (Get-Content -LiteralPath $file -Raw).Replace('/hongmeng_other_hours_ux_v134151?', '/hongmeng_other_hours_ux_v134151_release?').Replace('S3_BUCKET=other-hours-ux-qa','S3_BUCKET=other-hours-ux-release-qa')
      Set-Content -LiteralPath $file -Value $contents -Encoding utf8
    }
    $env:S3_BUCKET = 'other-hours-ux-release-qa'
  ...
  if ($Stage -eq 'migrate') {
    ...
    & node node_modules/next/dist/bin/next dev --hostname 127.0.0.1 --port 3122
  } elseif ($Stage -eq 'start') {
    if (!$Image -or $Image -notmatch '@sha256:[a-f0-9]{64}$') { throw 'An immutable image digest is required' }
    Docker @('run','-d','--name',$app,'--label',"codex.task=$taskLabel",'--network',$network,'-p','127.0.0.1:3122:3000','--env-file',$containerEnv,$Image) | Out-Null
  } elseif ($Stage -eq 'verify') {
    $health = Invoke-RestMethod 'http://127.0.0.1:3122/api/health'
    $ready = Invoke-RestMethod 'http://127.0.0.1:3122/api/ready'
    ...
    @{ image=$Image; revision=$ExpectedRevision; health=$health; ready=$ready; database=$database; verifiedAt=(Get-Date).ToString('o') } | ConvertTo-Json -Depth 8 | Set-Content -Encoding UTF8 output/playwright/other-hours-ux-v134151/runtime-verification.json
