...
try{
 ...
 writeFileSync(file,`async page=>{
  ...
  try{
   ...
   await page.locator('.sl-product-card').filter({has:page.getByRole('heading',{name:f.model,exact:true})}).click();await loaded();await page.getByLabel('返回样品列表').click();check(await page.getByLabel('搜索样品型号').inputValue()==='d014503 8305','return preserves fuzzy search');check(await page.evaluate(()=>new URL(window.location.href).searchParams.get('customer'))===f.customer,'return preserves customer');
   ...
   await page.setViewportSize({width:1366,height:1024});await page.goto(origin+'/weekly-plan-center?branch=samples&sampleView=ALL');await page.getByLabel('样品库二维码',{exact:true}).click();await page.getByRole('dialog',{name:'手机样品库二维码'}).waitFor();await shot('planning-qr-1366');await page.getByLabel('关闭二维码').click();check(await page.evaluate(()=>window.location.pathname)==='/weekly-plan-center','closing planning QR retains planning context');
   const planSearch=page.waitForResponse(r=>r.url().includes('/api/sample-tasks?')&&r.url().includes('keyword=014503')&&r.status()===200);await page.getByLabel('搜索样品任务',{exact:true}).fill('014503');await planSearch;await page.waitForFunction(()=>document.querySelector('[aria-label="样品计划表"]')?.getAttribute('aria-busy')==='false'&&document.querySelectorAll('.sp-table-scroll tbody tr').length>5);const planCustomer=page.waitForResponse(r=>r.url().includes('/api/sample-tasks?')&&r.url().includes('customer=')&&r.status()===200);await page.getByLabel('客户筛选',{exact:true}).selectOption(f.customer);await planCustomer;await page.waitForFunction(()=>document.querySelector('[aria-label="样品计划表"]')?.getAttribute('aria-busy')==='false');await page.locator('.sp-table-scroll').evaluate(el=>el.scrollTop=180);const planScroll=await page.locator('.sp-table-scroll').evaluate(el=>el.scrollTop);
   await page.getByRole('button',{name:'手机样品库',exact:true}).click();const planningReference=page.getByRole('dialog',{name:'手机样品库预览',exact:true});await planningReference.waitFor();const planFrame=page.frameLocator('iframe[title="手机样品参考库"]');await planFrame.locator('.sl-product-card').first().waitFor();await shot('planning-library-1366');await planFrame.getByLabel('返回入口').click();await planningReference.waitFor({state:'detached'});check(await page.getByLabel('搜索样品任务',{exact:true}).inputValue()==='014503'&&await page.getByLabel('客户筛选',{exact:true}).inputValue()===f.customer,'planning reference keeps search and customer');check(Math.abs(await page.locator('.sp-table-scroll').evaluate(el=>el.scrollTop)-planScroll)<5,'planning reference keeps table scroll');check(await page.evaluate(()=>window.location.pathname)==='/weekly-plan-center','library return stays on original planning page');
