...
try {
  const code = `async page => {
    ...
    try {
      ...
      await page.getByRole('region',{name:'分配负责人',exact:true}).waitFor({state:'detached'});
      await page.setViewportSize({width:1366,height:1024});
      ...
      check(await page.getByRole('button',{name:'转交负责人',exact:true}).count()===0&&await page.getByRole('button',{name:'更新交期 / 到料',exact:true}).count()===0,'acceptance does not grant assignment or arrival-editing permissions');
      await login('operator',followPath);
      await waitFollowSelection();
      await page.getByRole('button',{name:'转交负责人',exact:true}).click();
      await page.getByLabel('选择负责人',{exact:true}).selectOption(f.users.operator.id);
