...
const create = async type => (await req('create '+type, '/api/sample-tasks', { taskType: type, customerName: tag, productName: 'Sample cable', specification: tag+'-'+type, customerLevelCode: 'A', sampleQuantity: 5, planWeekStartDate: week, issuedDate: today, dueDate: today, planRemark: 'Isolated sample branch verification' })).task;
...
await req('repeat cannot save process data', `/api/sample-tasks/${repeat.id}/entries`, { kind: 'NOTICE', label: 'forbidden', payload: { text: 'none' }, mutationId: randomUUID() }, 409);
...
await req('download extended import template','/api/sample-tasks/import/template');
