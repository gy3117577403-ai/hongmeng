...
assertTextIncludes('app/api/native/work-orders/[id]/route.ts', 'const expected = `${old.code} CONFIRM`', 'native work order delete confirmation check');
assertTextIncludes('app/api/native/resource-files/[id]/route.ts', "trim() !== 'DELETE'", 'native resource delete confirmation check');
assertTextIncludes('app/api/native/connector-parameters/[id]/route.ts', "trim() !== 'DELETE'", 'native connector delete confirmation check');
assertTextIncludes('app/api/native/connector-parameters/batch/route.ts', "action === 'delete'", 'native connector batch delete branch check');
assertTextIncludes('app/api/native/connector-parameters/batch/route.ts', "trim() !== 'DELETE'", 'native connector batch delete confirmation check');
assertTextIncludes('app/api/native/connector-parameter-files/[id]/route.ts', "trim() !== 'DELETE'", 'native connector attachment delete confirmation check');
assertTextIncludes('app/api/native/connector-parameter-import-batches/[id]/rollback/route.ts', "trim() !== 'ROLLBACK'", 'native import batch rollback confirmation check');
assertTextIncludes('app/api/native/connector-parameters/import/commit/route.ts', "trim() !== 'IMPORT_CONFIRM'", 'native large import confirmation check');
assertTextIncludes('app/api/native/users/[id]/route.ts', "data.isActive ? 'ENABLE' : 'DISABLE'", 'native user active-state confirmation token check');
assertTextIncludes('app/api/native/users/[id]/route.ts', 'activeCount <= 1', 'native last active user protection check');
assertTextIncludes('app/api/native/users/[id]/reset-password/route.ts', "trim() !== 'RESET_PASSWORD'", 'native reset password confirmation check');
assertTextIncludes('harmony-tablet/entry/src/main/ets/stores/WorkOrderStore.ets', 'deleteWorkOrder(order.id, `${order.code} CONFIRM`)', 'Harmony work order delete confirmation call check');
assertTextIncludes('harmony-tablet/entry/src/main/ets/services/resourceApi.ets', "confirmText: 'DELETE'", 'Harmony resource delete confirmation call check');
assertTextIncludes('harmony-tablet/entry/src/main/ets/services/connectorApi.ets', "confirmText: action === 'delete' ? 'DELETE' : ''", 'Harmony connector batch delete confirmation call check');
assertTextIncludes('harmony-tablet/entry/src/main/ets/services/connectorApi.ets', "confirmText: 'ROLLBACK'", 'Harmony import rollback confirmation call check');
