...
try {
 ...
 const compatible=(await req('older account form preserves independent mobile access','/api/users/module-access',{id:reader.id,displayName:reader.displayName,accountStatus:'ACTIVE',modulePermissions:{},workbenchEnabled:false,fieldReportEnabled:false,expectedUpdatedAt:reader.updatedAt})).user;
 check(compatible.accessMethods.sampleLibrary,'omitted mobile option does not revoke access');
 ...
 const ranked=await req('exact model is first','/api/sample-library?q=D0145038305V01&customer='+encodeURIComponent(customer));check(ranked.items[0].id===p.id,'exact match precedes prefix');check(ranked.items[0].tasks===2,'empty repeat is not counted as another reference collection');
