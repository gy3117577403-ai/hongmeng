param(
  ...
  [ValidatePattern('^hongmeng_drawing_library_v134146_release(?:_[a-z0-9]+)?$')][string]$DatabaseName = 'hongmeng_drawing_library_v134146_release',
  [ValidatePattern('^hm-drawing-library-v134146-release-app(?:-[a-z0-9]+)?$')][string]$AppName = 'hm-drawing-library-v134146-release-app'
)
...
if ($Instance -eq 'second') {
  if (!$PSBoundParameters.ContainsKey('DatabaseName')) { $DatabaseName = 'hongmeng_drawing_library_v134146_release_second' }
  if (!$PSBoundParameters.ContainsKey('AppName')) { $AppName = 'hm-drawing-library-v134146-release-app-second' }
  if ($DatabaseName -ne 'hongmeng_drawing_library_v134146_release_second' -or $AppName -ne 'hm-drawing-library-v134146-release-app-second') {
    throw 'Second instance requires its assigned database and application container names.'
  }
} elseif ($DatabaseName -eq 'hongmeng_drawing_library_v134146_release_second' -or $AppName.EndsWith('-second')) {
  throw 'Names reserved for the second instance require -Instance second.'
}
$appPort = if ($Instance -eq 'second') { 3117 } else { 3116 }
$postgresPort = if ($Instance -eq 'second') { 55447 } else { 55446 }
$minioPort = if ($Instance -eq 'second') { 19054 } else { 19052 }
$minioConsolePort = $minioPort + 1
$networkSubnet = if ($Instance -eq 'second') { '10.249.147.0/24' } else { '10.249.146.0/24' }
$taskLabel = "drawing-library-v134146-release$instanceSuffix"
$networkName = "hm-drawing-library-v134146-release-net$instanceSuffix"
$postgresName = "hm-drawing-library-v134146-release-postgres$instanceSuffix"
$minioName = "hm-drawing-library-v134146-release-minio$instanceSuffix"
