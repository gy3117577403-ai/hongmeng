...
export default function MaterialLibraryMobileCapture({ code, user }: { code: string; user: CurrentUserDTO }) {
  ...
  async function complete() {
    ...
    if (!saved) return;
    if (!window.confirm(`确认归档本次 ${saved.photos.length} 张来料照片及检验数据？归档后可由有权限的人员在照片管理中更正。`)) return;
    setSaving(true);
    try {
  ...
  return <main className="material-mobile-page">
    ...
    {session.status === 'COMPLETED' ? <section className="material-mobile-completed"><span><CheckCircle2 size={34} /></span><h2>本次记录已归档</h2><p>{session.photos.length} 张来料照片和检验数据已同步到物料档案。{session.uploadMode === 'PERMANENT' ? '下次扫描永久码将新建一轮录入。' : '本临时二维码已失效。'}</p><Link href="/workspace/material-library"><PackageOpen size={16} />返回物料库</Link></section> : <>
      <section className="material-mobile-card material-mobile-camera">
        <header><div><span>01</span><div><strong>拍摄来料实物</strong><small>选取差异明显的角度，支持连续拍照或相册多选</small></div></div><em>{photoCount} 张</em></header>
        <MaterialPhotoUploadQueue key={session.id} sessionId={session.id} supplierVariantId={session.supplierVariantId} batchNumber={session.draftBatchNumber} disabled={form.supplierVariantId !== (session.supplierVariantId || '') || form.batchNumber !== (session.draftBatchNumber || '')} camera onBusyChange={setUploading} onComplete={async () => {
          const response = await fetch(`/api/material-library/sessions/${session.id}`, { cache: 'no-store' });
          const body = await bodyJson(response);
          if (!response.ok) throw new Error(body.error || '照片列表同步失败');
          setSession(body.session);
        }} />
        <div className="material-mobile-photo-grid">
          {(showAllPhotos ? session.photos : session.photos.slice(0, 6)).map((photo, index) => <button type="button" key={photo.id} onClick={() => setPreview(photo)}><Image unoptimized priority={index === 0} src={photo.thumbnailUrl || photo.contentUrl} width={photo.width || 320} height={photo.height || 240} alt={photo.originalName} style={{ transform: `rotate(${photo.rotation}deg)` }} /><span>{photo.isCover ? '封面' : index + 1}</span></button>)}
        </div>

        {session.photos.length > 6 && <button type="button" className="material-mobile-photo-more" onClick={() => setShowAllPhotos(value => !value)}>{showAllPhotos ? '收起照片' : `查看全部 ${session.photos.length} 张照片`}</button>}
      </section>
      ...
      <section className="material-mobile-card material-mobile-form">
        ...
        {session.item.supplierVariants.length > 1 && <label><span>本次供应商型号</span><select disabled={uploading} value={form.supplierVariantId} onChange={event => selectVariant(event.target.value)}>{session.item.supplierVariants.map(variant => <option key={variant.id} value={variant.id}>{variant.supplierName || '未登记供应商'} ｜ {variant.manufacturerModel || variant.supplierPartNumber || '未登记型号'}</option>)}</select></label>}
        ...
        <label><span>来料批次</span><input disabled={uploading} value={form.batchNumber} onChange={event => update('batchNumber', event.target.value)} placeholder="扫描批次标签或手工填写" /></label>
      ...
      <section className="material-mobile-archive-note"><Archive size={18} /><span><strong>确认后形成物料历史证据</strong><small>归档会把本次照片、规格、批次和不良警示同步到物料档案；误传照片可由有权限的人员移入照片回收站。</small></span></section>
    ...
    {preview && <div className="material-mobile-preview" onClick={() => setPreview(null)}><button type="button" aria-label="关闭预览" onClick={() => setPreview(null)}><X /></button><Image unoptimized src={preview.previewUrl || preview.contentUrl} width={preview.width || 960} height={preview.height || 720} alt={preview.originalName} style={{ transform: `rotate(${preview.rotation}deg)` }} />{active && <button className="delete" type="button" onClick={event => { event.stopPropagation(); void removePhoto(preview); }}><Trash2 size={16} />移除误拍</button>}</div>}
