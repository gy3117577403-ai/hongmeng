...
export default function HomeSampleTasks({ state, onChange, onCounts, onNavigate, refreshKey }: { state: SampleHomeState; onChange: (state: SampleHomeState) => void; onCounts: (counts: SampleHomeCounts) => void; onNavigate: (scrollTop: number) => void; refreshKey?: string }) {
  ...
  return <div className="hm-hcc-message-pane sample-home-pane">
    ...
    <div ref={scroll} className="hm-hcc-message-list hm-scroll-region sample-home-models" aria-busy={loading}>
      {loading ? <p className="sample-home-empty">正在加载样品任务…</p> : items.map(item => <Link className={`sample-home-model warning-${sampleWarning(item).kind.toLowerCase()}`} key={item.id} href={`/weekly-plan-center?branch=samples&taskId=${encodeURIComponent(item.id)}&sampleView=${state.view}&sampleSearch=${encodeURIComponent(state.keyword)}&from=home`} prefetch={false} onClick={() => onNavigate(scroll.current?.scrollTop || 0)} title={`${item.specificationSnapshot} · ${item.customerNameSnapshot} · ${item.code} · ${sampleWarning(item).label}`}><span>{item.specificationSnapshot}</span><ChevronRight size={16}/></Link>)}
