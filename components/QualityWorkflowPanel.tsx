...
import { qualityTaskCauses, qualityOperators, type QualityOperatorAssignments } from '@/lib/quality-direct-shared';
...
function TaskCard({ task, report, user, run, busy, users, onUploaded }: { task: InternalQualityRiskTaskDTO; report: InternalQualityRiskDTO; user: CurrentUserDTO; run: Action; busy: boolean; users: QualityAssignee[]; onUploaded: (file: File, taskId: string) => Promise<void> }) {
  ...
  const taskDraft = useQualityDraft(user.id + ':task:' + task.id + ':' + (report.reviewRound || 0), { result: task.result || '', actionTaken: task.actionTaken || '', occurrenceCause: causes.occurrenceCause, rootCause: causes.rootCause, operators: qualityOperators(task.operators) });
  ...
  const setField = (field: 'result' | 'actionTaken' | 'occurrenceCause' | 'rootCause', value: string) => taskDraft.setValue(current => ({ ...current, [field]: value }));
  async function saveTask(action: string) { const submitted = { result, actionTaken, occurrenceCause, rootCause, operators }; if (await run(action, { taskId: task.id, ...submitted, operatorIds: operators.map(item => item.id) })) taskDraft.saved(submitted); }
...
export default function QualityWorkflowPanel({ report, user, users, reviewMode = false, onUpdated, onEditDraft, embedded = false }: {
  ...
  return <div className={`qv3-panel ${reviewMode ? 'qv3-review-layout' : ''}`}>
    ...
    {(reviewMode || !direct && !legacy && report.status !== 'DRAFT') && <section className="qv3-card qv3-analysis" id="quality-analysis"><header><h3>原因与解决方案</h3>{reviewMode && <label>提交轮次<select value={review?.round || ''} onChange={event => setSelectedRound(Number(event.target.value))}>{report.reviews?.map(item => <option key={item.id} value={item.round}>第 {item.round} 轮 · {item.decision === 'PENDING' ? '待确认' : item.decision === 'APPROVED' ? '通过' : '退回'}</option>)}</select></label>}</header>
    ...
    {reviewMode && review && <><section className="qv3-card qv3-review-tasks"><h3>本轮提交的责任任务与照片</h3>{tasks.map(task => <details key={task.id}><summary><b>{task.ownerName}</b> · {task.result || '无处理结果'}</summary>{task.analysis && <div className="qv3-form-pair qv3-readonly"><section><strong>发生原因</strong><p>{qualityTaskCauses(task, report).occurrenceCause}</p></section><section><strong>根本原因</strong><p>{qualityTaskCauses(task, report).rootCause}</p></section></div>}{qualityOperators(task.operators).length > 0 && <div className="qd-task-operators"><strong>作业人员</strong><QualityOperatorTags people={qualityOperators(task.operators)} /></div>}<p><b>措施：</b>{task.actionTaken || '旧记录未单列'}</p><p><b>结果：</b>{task.result}</p><QualityEvidenceGallery attachments={photos.filter(item => item.taskId === task.id)} /></details>)}</section>
