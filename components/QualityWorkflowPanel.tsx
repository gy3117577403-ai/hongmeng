'use client';
import QualityNotificationHistory from '@/components/QualityNotificationHistory';
...
export default function QualityWorkflowPanel({ report, user, users, reviewMode = false, onUpdated, onEditDraft, embedded = false }: {
  ...
  return <div className={`qv3-panel ${reviewMode ? 'qv3-review-layout' : ''}`}>
    ...
    <QualityNotificationHistory report={report} canRetry={user.laborRole === 'ADMIN' || user.access.capabilities.includes('QUALITY:UPDATE')} busy={busy} onRetry={(notificationId, confirmResend) => void run('RETRY_NOTIFICATION', { notificationId, confirmResend })} />
  </div>;
}
