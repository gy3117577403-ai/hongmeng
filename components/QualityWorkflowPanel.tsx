...
function TaskCard({ task, report, user, run, busy, users, onUploaded }: { task: InternalQualityRiskTaskDTO; report: InternalQualityRiskDTO; user: CurrentUserDTO; run: Action; busy: boolean; users: QualityAssignee[]; onUploaded: (file: File, taskId: string) => Promise<void> }) {
  const taskDraft = useQualityDraft(user.id + ':task:' + task.id + ':' + (report.reviewRound || 0), { result: task.result || '', actionTaken: task.actionTaken || '' });
  const { result, actionTaken } = taskDraft.value;
  const setResult = (result: string) => taskDraft.setValue(current => ({ ...current, result }));
  const setActionTaken = (actionTaken: string) => taskDraft.setValue(current => ({ ...current, actionTaken }));
  async function saveTask(action: string) { const submitted = { result, actionTaken }; if (await run(action, { taskId: task.id, ...submitted })) taskDraft.saved(submitted); }
  ...
  return <details open={own || undefined} className={`qv3-task ${own ? 'is-own' : ''}`} id={`task-${task.id}`}>
    <summary><span className="qv3-avatar"><Users size={19} /></span><div><h3>{task.ownerName || '待指派'} {own && <em>我的任务</em>}</h3><small>{task.department} · {task.ownerUserId === report.ownerUserId ? '牵头汇总' : '独立负责'}</small></div><b className={`qv3-status ${task.status.toLowerCase()}`}>{taskLabels[task.status]}</b></summary>
...
export default function QualityWorkflowPanel({ report, user, users, reviewMode = false, onUpdated, onEditDraft, embedded = false }: {
  ...
  async function run(action: string, payload: Record<string, unknown> = {}) {
    ...
    try {
      ...
      onUpdated(body.report); if (['SAVE_ANALYSIS', 'SUBMIT_REVIEW'].includes(action)) analysisDraft.saved(payload as Record<string, string>); if (['SAVE_REVIEW', 'APPROVE'].includes(action)) reviewDraft.saved({ result: String(payload.result || '') }); setNotice(action === 'SUBMIT_REVIEW' ? '已提交品质确认，等待指定人员验证。' : action === 'COMPLETE_TASK' ? '你的处理结果已提交。整份异常仍按流程继续。' : '已保存');
      return true;
  ...
  return <div className={`qv3-panel ${reviewMode ? 'qv3-review-layout' : ''}`}>
    {!embedded && <div className="qv3-context"><div><small>{report.reportNo} · {reviewMode ? '品质确认' : '异常工单'}</small><h2>{qualityEventTitle(report)}</h2></div><span>牵头 <b>{report.ownerName || '待指定'}</b> · 品质 <b>{report.reviewerName || '待指定'}</b></span></div>}
    <nav className="qv3-progress" aria-label="异常处理阶段">{Object.entries(QUALITY_PHASE_LABELS).map(([key, label], index) => <span aria-current={flow.phase === key ? 'step' : undefined} className={flow.phase === key ? 'current' : ''} key={key}><b>{index + 1}</b>{label}</span>)}</nav>
    ...
    {error && <div role="alert" className="qv3-error">{error}<button disabled={refreshing || busy} onClick={() => void refreshRecord()}>读取最新记录（保留草稿）</button></div>}{notice && <p role="status" className="qv3-notice">{notice}</p>}
    ...
    {(reviewMode || !legacy && report.status !== 'DRAFT') && <section className="qv3-card qv3-analysis" id="quality-analysis"><header><h3>原因与解决方案</h3>{reviewMode && <label>提交轮次<select value={review?.round || ''} onChange={event => setSelectedRound(Number(event.target.value))}>{report.reviews?.map(item => <option key={item.id} value={item.round}>第 {item.round} 轮 · {item.decision === 'PENDING' ? '待确认' : item.decision === 'APPROVED' ? '通过' : '退回'}</option>)}</select></label>}</header>
