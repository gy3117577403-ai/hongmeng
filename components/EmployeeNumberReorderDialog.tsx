...
function requestItems(rows: DraftRow[]) {
...
function downloadBatch(batch: ReorderBatch): void {
  const lines = [
    ['顺序', '原员工编号', '新员工编号', '员工姓名', '部门', '岗位', '班组', '人员类型'],
...
export default function EmployeeNumberReorderDialog({
  ...
  function applyBulkRows(): void {
    ...
    if (!parsed.length) {
      setError('没有识别到补录人员；请按“姓名、部门、岗位、班组”粘贴');
      return;
    }
  ...
  return (
    <div className="hr-reorder-backdrop" role="presentation" onMouseDown={event => {
      ...
      <div ref={layerRef} className="hr-reorder-dialog" role="dialog" aria-modal="true" aria-labelledby="hr-reorder-title" tabIndex={-1}>
        ...
        <div className="hr-reorder-body hm-scroll-region">
          {phase === 'edit' && <>
            ...
            <section className="hr-reorder-toolbar">
              <div><b>最终编号顺序</b><small>从 0001 开始，使用上下按钮调整人事确认顺序</small></div>
              <div>
            ...
            {bulkOpen && <section className="hr-reorder-bulk">
              <label><span>从 Excel 复制后直接粘贴</span><small>列顺序：姓名、部门、岗位、班组；支持制表符或逗号。</small><textarea value={bulkText} onChange={event => setBulkText(event.target.value)} placeholder={'姓名\t部门\t岗位\t班组\n张三\t生产部\t操作员\t装配'} /></label>
            ...
            <section className="hr-reorder-table" aria-label="员工编号重排名单">
              <header><span>顺序/目标编号</span><span>原编号/类型</span><span>员工姓名</span><span>部门</span><span>岗位</span><span>班组</span><span>调整</span></header>
              <div>
          ...
          {phase === 'preview' && preview && <>
            ...
            <section className="hr-reorder-preview-list">
              ...
              <div>{preview.rows.map((row, index) => <article key={row.key}>
                <span>{index + 1}</span><span><b>{row.name}</b><small>{row.department || '部门未维护'} · {row.position || '岗位未维护'}</small></span><span>{row.oldEmployeeNo || '补录新增'}</span><strong>{row.newEmployeeNo}</strong><em className={row.changed ? 'changed' : ''}>{row.kind === 'NEW' ? '新建档案' : row.changed ? '编号变更' : '保持不变'}</em>
              </article>)}</div>
            </section>
        ...
        <footer>
          {phase === 'edit' && <><button type="button" className="hr-secondary-button" onClick={onClose}>取消</button><button type="button" className="hr-primary-button" disabled={previewing || invalidNewCount > 0 || rows.length === 0} onClick={() => void createPreview()}>{previewing ? <Loader2 className="spin" /> : <ListOrdered />}{previewing ? '正在校验…' : '生成变更预览'}</button></>}
          {phase === 'preview' && preview && <><button type="button" className="hr-secondary-button" disabled={committing} onClick={() => { setPhase('edit'); setConfirmationText(''); setError(''); }}>返回调整</button><button type="button" className="hr-primary-button danger" disabled={committing || confirmationText !== preview.confirmationText || !preview.hasChanges} onClick={() => void commit()}>{committing ? <Loader2 className="spin" /> : <ShieldCheck />}{committing ? '正在事务执行…' : '确认并执行重排'}</button></>}
