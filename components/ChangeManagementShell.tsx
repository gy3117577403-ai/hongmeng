        ...
        <div className="change-workspace">
          ...
          <aside ref={contextRef} className={`change-context ${compactContext && contextOpen ? 'open' : ''}`} aria-label="变更上下文">
            ...
            {!selected ? <div className="change-context-empty"><Info /><p>选择变更后查看责任、关联来源和附件。</p></div> : <div className="change-context-scroll hm-scroll-region">
              ...
              <section className="context-section"><h3><ClipboardList size={15} />关联来源</h3>{selected.sourceIssue ? <a className="context-link" href={`/workspace/issues?issueId=${encodeURIComponent(selected.sourceIssue.id)}`}><div><strong>{selected.sourceIssue.code}</strong><span title={selected.sourceIssue.title}>{selected.sourceIssue.title}</span></div><ExternalLink size={14} /></a> : <p className="context-muted">未关联来源问题</p>}{selected.workOrder ? <a className="context-link" href={`/dashboard?workOrderId=${encodeURIComponent(selected.workOrder.id)}`}><div><strong>{selected.workOrder.specification || selected.workOrder.code}</strong><span>{selected.workOrder.customerName || '客户未设置'} · {selected.workOrder.productName}</span></div><ExternalLink size={14} /></a> : <p className="context-muted">未关联生产工单</p>}</section>
