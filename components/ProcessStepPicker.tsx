...
export default function ProcessStepPicker({ steps, currentId, onSelect, desktop = false, disabled = false, batch = false, selected = [], onToggle, onAbnormal }:
  { steps: PickerStep[]; currentId?: string; onSelect: (id: string) => void; desktop?: boolean; disabled?: boolean; batch?: boolean;
  ...
  const visible = steps.filter(step => (!query || `${step.position} ${step.processName}`.toLowerCase().includes(query.toLowerCase())) &&
    (group === 'all' || String(step.sequenceGroup || 1) === group) && (filter === 'all' || filter === 'selected' ? filter !== 'selected' || selected.includes(step.id) : filter === 'unfinished' ? remaining(step) : !remaining(step)));
  useEffect(() => {
