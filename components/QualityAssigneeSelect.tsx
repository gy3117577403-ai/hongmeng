...
export function QualityPeopleFields({ ids, reviewer, users, onChange, operators = {} }: {
  ...
  return <section className="qv3-people qd-people"><header><strong>主要责任人 <b>*</b><small>已选 {ids.length} 人</small></strong><button type="button" onClick={() => setOpen(!open)} aria-expanded={open}><Plus size={16} />{open ? '完成选择' : '添加责任人'}</button></header>
    {open && <div className="qd-picker"><label className="qd-search"><Search size={16} /><input autoFocus aria-label="搜索责任人" placeholder="姓名、账号或部门" value={query} onChange={event => setQuery(event.target.value)} /></label><div className="qd-picker-results">{users.filter(user => user.id !== reviewer && `${user.displayName} ${user.username} ${user.department}`.toLowerCase().includes(query.toLowerCase())).map(user => <label key={user.id}><input type="checkbox" checked={ids.includes(user.id)} onChange={() => change(ids.includes(user.id) ? ids.filter(id => id !== user.id) : [...ids, user.id])} /><span><strong>{user.displayName || user.username}</strong><small>{user.department || user.username}</small></span></label>)}{!users.length && <p>暂无可用工作台账号，请联系管理员完善账号授权。</p>}</div></div>}
