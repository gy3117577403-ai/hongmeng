...
import { planMinutesText, planHoursText, planTimeSourceText, planTotalMilliseconds } from '@/lib/planning-time';
...
function totalDuration(milliseconds?: string | null): string {
  ...
  return Number.isFinite(number) ? planHoursText(number) : '待计算';
}
...
function planningUnitMilliseconds(order: ProductionPlanOrderDTO): number | null {
  return order.planningUnitMilliseconds || order.effectiveUnitMilliseconds || order.currentUnitMilliseconds || null;
}
            ...
            <aside className="planning-order-pool" role="dialog" aria-modal="true" aria-label="待安排订单池">
              ...
              <div className="planning-pool-list hm-scroll-region" tabIndex={0}>
                {orderPool.map(order => <article className={`priority-${order.priority}`} key={order.id}>
                  ...
                  <dl><div><dt>未排数量</dt><dd>{order.remainingQuantity.toLocaleString()}</dd></div><div><dt>客户交期</dt><dd>{order.customerDueDate ? order.customerDueDate.slice(5) : '待确认'}</dd></div><div><dt>单件工时</dt><dd>{planMinutesText(planningUnitMilliseconds(order))}</dd></div></dl>
    ...
    {orderDialog && <div ref={dialogRef} className="planning-dialog order-dialog" role="dialog" aria-modal="true" aria-labelledby="planning-order-dialog-title">
      ...
      <div className="planning-dialog-body">
        <div className="planning-form-grid">
          ...
          <label className="planning-total-time"><span>订单总工时</span><output>{orderDraftTotalMilliseconds ? planHoursText(orderDraftTotalMilliseconds) : '待维护'}</output><small>{orderDraftTotalMilliseconds ? '单件工时 × 订单数量' : '补齐单件工时后自动计算'}</small></label>
    ...
    {batchDialog && <div ref={dialogRef} className="planning-dialog batch-dialog" role="dialog" aria-modal="true" aria-labelledby="planning-batch-dialog-title">
      ...
      <div className="planning-dialog-body">
        <div className="planning-form-grid">
          ...
          <label className="wide planning-total-time"><span>本批总工时</span><output>{batchDraftTotalMilliseconds ? planHoursText(batchDraftTotalMilliseconds) : '待维护'}</output><small>{batchDraftTotalMilliseconds ? '单根工时 × 本批数量，保存后用于生产工时统计' : '可先排产进入生产待配置，开始工序前补齐工时'}</small></label>
