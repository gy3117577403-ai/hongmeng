...
function emptyOrderForm(): OrderForm {
  ...
  return {
    sourceOrderNo: '', sourceLineNo: '1', customerName: '', productName: '', specification: '',
...
function orderForm(order: ProductionPlanOrderDTO): OrderForm {
  return {
    ...
    sourceLineNo: String(order.sourceLineNo),
...
export default function PlanningCenterShell({ user }: { user: CurrentUserDTO }) {
  ...
  const filteredOrders = useMemo(() => {
    ...
    return orders.filter(order => {
      ...
      if (!word) return true;
      return [order.sourceOrderNo, order.customerName, order.productName, order.specification, order.remark || '']
  ...
  const scheduleRows = allBatches.filter(({ order, batch }) => {
    ...
    if (priority !== 'all' && order.priority !== priority) return false;
    const word = keyword.trim().toLocaleLowerCase();
    return !word || [order.sourceOrderNo, order.customerName, order.productName, order.specification].some(value => value.toLocaleLowerCase().includes(word));
  });
  ...
  async function deleteOrder(order: ProductionPlanOrderDTO): Promise<void> {
    if (!window.confirm(`确认删除订单 ${order.sourceOrderNo}-${order.sourceLineNo}？仅未下达订单可以删除。`)) return;
    ...
    <main ref={mainRef} className="planning-center-shell hm-workbench-root hm-workbench-navigation-overlay">
      ...
      <div className="planning-center-main">
        ...
        <section className="planning-toolbar" aria-label="计划筛选和操作">
          <label className="planning-search"><Search size={17} aria-hidden="true" /><input value={keyword} onChange={event => setKeyword(event.target.value)} placeholder="搜索订单号、客户、规格或品名" /></label>
          ...
          <aside className="planning-order-pool">
            ...
            <div className="planning-pool-list hm-scroll-region" tabIndex={0}>
              {orderPool.map(order => <article className={`priority-${order.priority}`} key={order.id}>
                <div className="planning-pool-order"><span>{order.sourceOrderNo}-{order.sourceLineNo}</span><em>{priorityText(order.priority)}</em></div>
              ...
              <table className="planning-table">
                ...
                <tbody>{scheduleRows.map(({ order, batch }) => <Fragment key={batch.id}>
                  <tr className={`state-${batch.releaseState} ${expandedOrderId === batch.id ? 'expanded' : ''}`}>
                    ...
                    <td><button className="planning-product-link" type="button" title={`${order.specification} · ${order.productName}`} onClick={() => setExpandedOrderId(current => current === batch.id ? '' : batch.id)}><strong>{order.specification}</strong><span>{order.customerName} · {order.productName}</span><small>{order.sourceOrderNo}-{order.sourceLineNo} / 第 {batch.batchNo} 批</small></button></td>
                  ...
                  {expandedOrderId === batch.id && <tr className="planning-inspector-row" key={`${batch.id}-detail`}><td colSpan={12}><div className="planning-inline-inspector">
                    <div><span>订单信息</span><strong>{order.sourceOrderNo} · 行 {order.sourceLineNo}</strong><small>{order.remark || '无备注'}</small></div>
