...
function hoursMetric(rows: Array<{ attendanceMilliseconds: number; standardLaborMilliseconds: number; exemptAbnormalMilliseconds: number; otherWorkMilliseconds?: number; attainmentNumeratorMilliseconds?: number; attainmentCapacityMilliseconds: number; attainmentIncompleteDays?: number; attainmentDataComplete?: boolean }>): MetricDefinition {
  ...
  return { label: '工时达成率', value: percentText(rate), description: total.complete ? `参与达成：计入工时 ${compactHours(total.numerator)} ÷ 目标工时 ${compactHours(total.denominator)}（出勤 × 95%）；右侧为筛选人员全部工时` : '参与达成人员考勤待完善，完成工时已保留', tone: 'orange', stats: [
    ...
    { label: '已确认损耗工时', value: compactHours(total.loss), note: '全额计入' },
    { label: '其他工时', value: compactHours(total.other), note: '已审批通过，全额计入' },
  ] };
}
...
function branchMethod(branch: ReportBranchKey): string {
  ...
  if (['attendance-attainment', 'team-hours', 'employee-attainment', 'employee-matrix', 'unmatched-labor'].includes(branch)) {
    ...
    return '达成率=（完成+已确认损耗+其他工时）÷（出勤×0.95）；出勤含加班。报工按实际生产日计入，待前序覆盖不影响；缺考勤时达成率为 —。';
  }
...
function metricForBranch(
  ...
  const common: Record<ReportBranchKey, MetricDefinition> = {
    ...
    'team-hours': { label: '工时达成率', value: percentText(operationsSummary?.attainmentBasisPoints), description: '按班组汇总参与达成员工的工时', tone: 'orange', stats: [
      ...
      { label: '已确认损耗工时', value: compactHours(operationsSummary?.exemptAbnormalMilliseconds), note: '全额计入' },
      { label: '其他工时', value: compactHours(operationsSummary?.otherWorkMilliseconds), note: '已审批通过' },
    ] },
    'employee-attainment': { label: '工时达成率', value: percentText(employeeSummary?.attainmentBasisPoints), description: '（完成 + 已确认损耗 + 其他工时）÷（出勤 × 95%）', tone: 'orange', stats: [
      ...
      { label: '已确认损耗工时', value: compactHours(employeeSummary?.exemptAbnormalMilliseconds), note: '全额计入' },
      { label: '其他工时', value: compactHours(employeeSummary?.otherWorkMilliseconds), note: '已审批通过' },
    ] },
...
export default function ReportCenterBranchDashboard({
  ...
  async function exportBranch(): Promise<void> {
    ...
    if (initialBranch === 'quantity-attainment' || initialBranch === 'production-trend') {
      ...
      rows = [
        ['班组', '员工数', '正常出勤（小时）', '加班（小时）', '总出勤（小时）', '完成工时（小时）', '已确认损耗（小时）', '其他工时（小时）', '目标工时（出勤×95%）', '达成率', '考勤完整'],
      ...
      rows = [
        ['班组', '员工编号', '员工', '日期', '考勤状态', '正常出勤（小时）', '加班（小时）', '总出勤（小时）', '完成工时（小时）', '已确认损耗（小时）', '其他工时（小时）', '目标工时（出勤×95%）', '达成率'],
      ...
      rows = [
        ['员工编号', '姓名', '班组', '正常出勤（小时）', '加班（小时）', '总出勤（小时）', '完成工时（小时）', '已确认损耗（小时）', '其他工时（小时）', '目标工时（出勤×95%）', '达成率', '考勤待完善天数'],
    ...
    await downloadWorkbook({
      ...
      detailSheets: initialBranch === 'employee-attainment' ? [{
        ...
        name: '每日工时明细',
        headers: ['日期', '员工编号', '姓名', '班组', '考勤状态', '正常出勤（小时）', '实际加班（小时）', '总出勤（小时）', '完成工时（小时）', '已确认损耗（小时）', '其他工时（小时）', '目标工时（出勤×95%）', '达成率', '数据说明', '统计分账', '口径来源'],
...
function EmployeeDrawer({ row, onClose }: { row: EmployeeAttainmentRowDTO; onClose: () => void }) {
  ...
  return <div className="report-focus-drawer-backdrop" role="presentation" onMouseDown={event => { if (event.target === event.currentTarget) onClose(); }}><aside className="report-focus-drawer report-employee-drawer" role="dialog" aria-modal="true" aria-label={`${row.employee.name}每日达成详情`}><header><div><small>员工工时达成</small><h2>{row.employee.name}</h2><p>{row.employee.employeeNo} · {row.employee.team || row.employee.department || '未分组'} · {row.employee.position || '岗位未设置'}</p></div><button type="button" aria-label="关闭员工详情" onClick={onClose}><X /></button></header>
    <div className="report-employee-drawer-summary"><article><small>达成率</small><strong>{percentText(row.attainmentBasisPoints)}</strong><span>{row.attainmentDataComplete === false ? '考勤待完善，完成工时已保留' : hasPartialAttainment(row) ? `部分日期参与：计入工时 ${compactHours(row.attainmentNumeratorMilliseconds)} ÷ 目标工时 ${compactHours(row.attainmentCapacityMilliseconds)}` : '（完成 + 已确认损耗 + 其他工时）÷（出勤 × 95%）'}</span></article><article><small>总出勤</small><strong>{compactHours(row.attendanceMilliseconds)}</strong><span>正常 {compactHours(row.regularAttendanceMilliseconds)} + 加班 {compactHours(row.recognizedOvertimeMilliseconds)}</span></article><article><small>完成工时</small><strong>{compactHours(row.standardLaborMilliseconds)}</strong><span>待前序覆盖照常计入</span></article><article><small>已确认损耗</small><strong>{compactHours(row.exemptAbnormalMilliseconds)}</strong><span>全额计入</span></article><article><small>其他工时</small><strong>{compactHours(row.otherWorkMilliseconds)}</strong><span>已审批通过，全额计入</span></article><article><small>目标工时</small><strong>{compactHours(row.attainmentCapacityMilliseconds)}</strong><span>出勤 × 95%，预留 5% 休息余量</span></article></div>
    <section className="report-employee-day-section"><header><div><small>逐日证据</small><h3>考勤、加班、请假与报工明细</h3></div><span>{row.days.length} 天</span></header><div className="report-employee-day-list">{row.days.map(day => {
      ...
      return <details key={day.date} className={day.includedInAttainment ? 'included' : 'excluded'}>
        ...
        <div className="report-employee-day-metrics">
          ...
          {(day.otherWorkMilliseconds || 0) > 0 && <p><Link href={'/workspace/other-hours?employeeId=' + encodeURIComponent(row.employee.id) + '&from=' + day.date + '&to=' + day.date + '&status=APPROVED'}>查看当日其他工时事项、照片与审批轨迹（{day.otherWorkCount || 0} 笔）</Link></p>}
