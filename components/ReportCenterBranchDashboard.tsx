  ...
  async function exportBranch(): Promise<void> {
    ...
    await downloadWorkbook({
      ...
      detailSheets: initialBranch === 'employee-attainment' ? [{
        ...
        name: '每日工时明细',
        headers: ['日期', '员工编号', '姓名', '班组', '考勤状态', '正常出勤（小时）', '实际加班（小时）', '总出勤（小时）', '完成工时（小时）', '已确认损耗（小时）', '其他工时（小时）', '目标工时（出勤×95%）', '达成率', '数据说明'],
        rows: employeeRows.flatMap(row => row.days.map(day => [day.date, row.employee.employeeNo, row.employee.name, row.employee.team || '', day.attendanceStatus, exportHours(day.regularAttendanceMilliseconds), exportHours(day.recognizedOvertimeMilliseconds), exportHours(day.attendanceMilliseconds), exportHours(day.standardLaborMilliseconds), exportHours(day.exemptAbnormalMilliseconds), exportHours(day.otherWorkMilliseconds), exportHours(day.attainmentCapacityMilliseconds), percentText(day.targetAttainmentBasisPoints), day.date > todayKey() ? '未开始' : day.includedInAttainment ? day.date === todayKey() ? '今日统计中' : '已计入' : employeeExclusionText(day.exclusionReason)])),
      }] : undefined,
      kpis: [
