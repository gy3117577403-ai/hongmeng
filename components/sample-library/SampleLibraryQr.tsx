...
import { SampleLibraryReferenceDialog } from './SampleLibraryReference';
...
export default function SampleLibraryEntry({ productId, title, returnTo, onNavigate, preserveContext = false }: {productId?:string;title?:string;returnTo?:string;onNavigate?:()=>void;preserveContext?:boolean}) {
  const [open,setOpen]=useState(false),[browsing,setBrowsing]=useState(false);
  const params=new URLSearchParams();if(productId)params.set('product',productId);if(returnTo)params.set('returnTo',returnTo);
  return <>{preserveContext?<button className="hm-workbench-button" onClick={()=>setBrowsing(true)}>手机样品库</button>:<a className="hm-workbench-button" href={`/sample-library?${params}`} onClick={onNavigate}>手机样品库</a>}<button className="hm-workbench-button" aria-label="样品库二维码" onClick={()=>setOpen(true)}><QrCode size={17}/></button>{open&&<SampleLibraryQrDialog productId={productId} title={title} onClose={()=>setOpen(false)}/>}<SampleLibraryReferenceDialog open={browsing} productId={productId} context="planning" onClose={()=>setBrowsing(false)}/></>;
}
