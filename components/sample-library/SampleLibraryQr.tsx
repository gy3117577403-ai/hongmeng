...
export default function SampleLibraryEntry({ productId, title, returnTo }: {productId?:string;title?:string;returnTo?:string}) {
  ...
  return <><a className="hm-workbench-button" href={`/sample-library?${params}`}>手机样品库</a><button className="hm-workbench-button" aria-label="样品库二维码" onClick={()=>setOpen(true)}><QrCode size={17}/></button>{open&&<SampleLibraryQrDialog productId={productId} title={title} onClose={()=>setOpen(false)}/>}</>;
}
