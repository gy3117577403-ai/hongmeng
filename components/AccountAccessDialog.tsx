...
function accountDraft(account: UserDTO): Draft { return { id: account.id, employeeId: account.employeeId || '', username: account.username, displayName: account.displayName, accountStatus: account.accountStatus || (account.isActive ? 'ACTIVE' : 'DISABLED'), permissions: account.laborRole === 'ADMIN' || account.accessGrants?.some(grant => grant.isActive && grant.profileKey === 'ADMIN_GLOBAL') ? Object.fromEntries(BUSINESS_ACCESS_MODULES.map(module => [module.key, 'COLLABORATE' as const])) : account.moduleAccess?.permissions || {}, workbench: account.moduleAccess?.workbenchEnabled ?? account.accessMethods?.workbench !== false, field: Boolean(account.accessMethods?.fieldReport), password: '', expectedUpdatedAt: account.updatedAt, adopt: Boolean(account.moduleAccess) }; }
