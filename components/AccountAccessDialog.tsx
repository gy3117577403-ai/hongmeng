...
type Draft = { id: string; employeeId: string; username: string; displayName: string; accountStatus: string; permissions: ModulePermissions; workbench: boolean; field: boolean; library: boolean; delegate: boolean; password: string; expectedUpdatedAt: string; adopt: boolean };
const emptyDraft = (): Draft => ({ id: '', employeeId: '', username: '', displayName: '', accountStatus: 'ACTIVE', permissions: {}, workbench: true, field: false, library: false, delegate: false, password: '', expectedUpdatedAt: '', adopt: true });
function accountDraft(account: UserDTO): Draft { return { id: account.id, employeeId: account.employeeId || '', username: account.username, displayName: account.displayName, accountStatus: account.accountStatus || (account.isActive ? 'ACTIVE' : 'DISABLED'), permissions: account.laborRole === 'ADMIN' || account.accessGrants?.some(grant => grant.isActive && grant.profileKey === 'ADMIN_GLOBAL') ? Object.fromEntries(BUSINESS_ACCESS_MODULES.map(module => [module.key, 'COLLABORATE' as const])) : account.moduleAccess?.permissions || {}, workbench: account.moduleAccess?.workbenchEnabled ?? account.accessMethods?.workbench !== false, field: Boolean(account.accessMethods?.fieldReport), library: Boolean(account.accessMethods?.sampleLibrary), delegate: Boolean(account.accessMethods?.employeeAccountManager), password: '', expectedUpdatedAt: account.updatedAt, adopt: Boolean(account.moduleAccess) }; }
...
export default function AccountAccessDialog({ user, initialEmployeeId, onClose }: { user: CurrentUserDTO; initialEmployeeId?: string; onClose: () => void }) {
  const globalManager = isGlobalAccountManager(user);
  const canAuthorize = canAuthorizeEmployeeAccounts(user);
  ...
  const protectedAccount = Boolean(selected && (selected.id === user.id || selected.laborRole === 'ADMIN' || selected.accessGrants?.some(grant => grant.profileKey === 'ADMIN_GLOBAL' || !globalManager && grant.profileKey === 'EMPLOYEE_ACCESS_MANAGER')));
  ...
  const save = async () => {
    ...
    try {
      ...
      if (effectiveEdit) {
        response = await fetch('/api/users/module-access', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...draft, modulePermissions: draft.permissions, workbenchEnabled: draft.workbench, fieldReportEnabled: draft.field, sampleLibraryEnabled: draft.library, preserveBusinessGrants: !draft.adopt, ...(globalManager ? { employeeAccountManager: draft.delegate } : {}) }) });
      } else if (draft.id) {
      ...
      if (draft.id && draft.password && !effectiveEdit) {
      ...
      <div className={`aa-workspace ${draft ? 'is-editing' : ''}`}>
        ...
        <section className="aa-editor" aria-label="账号配置">{!draft ? <div className="aa-welcome"><div><ShieldCheck size={38} /></div><h2>让每个人拥有恰当的操作权限</h2><p>选择左侧员工，查看后台模块与扫码入口。<br />只读用于查看，协同用于处理业务。</p><div className="aa-welcome-modules">{BUSINESS_ACCESS_MODULES.map(module => { const Icon = moduleIcons[module.key]; return <span key={module.key}><Icon size={18} />{module.label}</span>; })}</div></div> : <>
          ...
          <div className="aa-editor-scroll">
            {protectedAccount && <div className="aa-notice">{selected?.id === user.id ? '当前登录账号：自身权限由另一位管理员维护。' : '管理员与授权管理人员的账号由管理员维护，此处只读展示。'}</div>}
            ...
            {tab === 'modules' && <>
              ...
              {!canAuthorize && <div className="aa-notice">你可以维护普通账号状态及密码；模块授权需另行开通“员工账号与业务授权管理”。</div>}
              ...
              {globalManager && !protectedAccount && <label className="aa-entry aa-delegation"><input type="checkbox" aria-label="员工账号与业务授权管理" checked={draft.delegate} disabled={busy} onChange={event => patch({ delegate: event.target.checked })} /><ShieldCheck /><span><strong>员工账号与业务授权管理</strong><small>需同时具有人事协同权限；可为普通员工开通业务模块、手机样品库和管理账号。不能修改自己、管理员或授权管理人员。</small></span></label>}
              {canAuthorize && !globalManager && <div className="aa-notice">已开通员工业务授权管理。可给普通员工配置模块；保存会记录操作人，并更新员工登录权限。</div>}
            </>}
            {tab === 'security' && <div className="aa-form"><h3>访问方式</h3><label className="aa-entry"><input type="checkbox" checked={draft.workbench} disabled={!effectiveEdit} onChange={event => patch({ workbench: event.target.checked, permissions: event.target.checked ? draft.permissions : {}, adopt: true })} /><Monitor /><span><strong>后台工作台</strong><small>使用已选择的业务模块</small></span></label><label className="aa-entry"><input type="checkbox" checked={draft.field} disabled={!effectiveEdit} onChange={event => patch({ field: event.target.checked, adopt: true })} /><QrCode /><span><strong>扫码报工</strong><small>生产岗位实名报工，与后台入口独立</small></span></label><label className="aa-entry"><input type="checkbox" aria-label="手机样品库只读" checked={draft.library} disabled={!effectiveEdit} onChange={event => patch({ library: event.target.checked })} /><QrCode /><span><strong>手机样品库 · 只读</strong><small>扫码查看历史照片与参数，独立于后台模块和报工</small></span></label><label><span><KeyRound size={15} />{draft.id ? '重置登录密码' : '初始登录密码'}</span><input type="password" autoComplete="new-password" placeholder={draft.id ? '留空保持现有密码' : '设置首次登录密码'} value={draft.password} disabled={protectedAccount} onChange={event => patch({ password: event.target.value })} /></label><div className="aa-notice">新设密码后首次登录需要修改；停用账号或更新访问权限会使旧登录失效。</div>{selected && <p className="aa-module-help">最近更新 {time(selected.updatedAt)} · 最近登录 {time(selected.lastLoginAt)}</p>}</div>}
          </div>
