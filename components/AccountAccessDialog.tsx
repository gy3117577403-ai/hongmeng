...
type Draft = { id: string; employeeId: string; username: string; displayName: string; accountStatus: string; permissions: ModulePermissions; workbench: boolean; field: boolean; library: boolean; password: string; expectedUpdatedAt: string; adopt: boolean };
const emptyDraft = (): Draft => ({ id: '', employeeId: '', username: '', displayName: '', accountStatus: 'ACTIVE', permissions: {}, workbench: true, field: false, library: false, password: '', expectedUpdatedAt: '', adopt: true });
function accountDraft(account: UserDTO): Draft { return { id: account.id, employeeId: account.employeeId || '', username: account.username, displayName: account.displayName, accountStatus: account.accountStatus || (account.isActive ? 'ACTIVE' : 'DISABLED'), permissions: account.laborRole === 'ADMIN' || account.accessGrants?.some(grant => grant.isActive && grant.profileKey === 'ADMIN_GLOBAL') ? Object.fromEntries(BUSINESS_ACCESS_MODULES.map(module => [module.key, 'COLLABORATE' as const])) : account.moduleAccess?.permissions || {}, workbench: account.moduleAccess?.workbenchEnabled ?? account.accessMethods?.workbench !== false, field: Boolean(account.accessMethods?.fieldReport), library: Boolean(account.accessMethods?.sampleLibrary), password: '', expectedUpdatedAt: account.updatedAt, adopt: Boolean(account.moduleAccess) }; }
  ...
  const save = async () => {
    ...
    try {
      ...
      if (effectiveEdit && draft.adopt) {
        response = await fetch('/api/users/module-access', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...draft, modulePermissions: draft.permissions, workbenchEnabled: draft.workbench, fieldReportEnabled: draft.field, sampleLibraryEnabled: draft.library }) });
      } else if (draft.id) {
      ...
      if (draft.id && effectiveEdit && !draft.adopt && draft.library !== Boolean(selected?.accessMethods?.sampleLibrary)) {
        const access = await fetch('/api/users/sample-library-access', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: draft.id, enabled: draft.library, expectedUpdatedAt: body.user?.updatedAt || draft.expectedUpdatedAt, password: draft.password }) });
        const result = await access.json(); if (!access.ok || !result.ok) throw new Error(result.error || '手机样品库权限保存失败，请刷新核对');
      }
