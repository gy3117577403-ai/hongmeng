...
export default function FinishedGoodsWorkbench({ user, initialData, initialQuery }: { user: CurrentUserDTO; initialData: FgWorkbench; initialQuery: string }) {
  ...
  return <main className={`fg-shell hm-workbench-root hm-workbench-navigation-overlay ${size === 16 ? 'fg-comfortable' : 'fg-compact'} ${continuous ? 'fg-continuous' : ''}`}>
    ...
    <section className="fg-toolbar">
      <label className="fg-search"><Search size={17}/><input aria-label="搜索工单、客户、规格" placeholder="工单 / 客户 / 规格" value={search} onChange={e => setSearch(e.target.value)}/>{search && <button onClick={() => setSearch('')} type="button" aria-label="清空搜索"><X size={14}/></button>}</label>
      {view !== 'legacy' && (view !== 'queue' || ['shipped','all'].includes(filter)) && <label className="fg-date-filter">{view === 'stock' || view === 'receipts' ? '入库日期' : '出库日期'}<input aria-label={view === 'stock' || view === 'receipts' ? '入库日期' : '出库日期'} title="待处理包含往日未完成记录" type="date" value={date} onChange={e => { setDate(e.target.value); setScope(view === 'stock' ? 'range' : 'day'); setPage(1); setBatchId(''); }}/></label>}
    ...
    <section className="fg-table-scroll" aria-label="成品与出货明细" aria-busy={loading}>
      <table className="fg-table"><colgroup>{['select','order','customer','spec','pending','stock','shipped','process','received','dispatched','status','actions'].map(name=><col key={name} className={`fg-col-${name}`}/>)}</colgroup>
        <thead><tr><th><input aria-label="全选本页可处理记录" type="checkbox" disabled={!selectableRows.length || busy || loading} checked={Boolean(selectableRows.length) && selectableRows.every(r => selected.includes(r.id))} onChange={e => setSelected(e.target.checked ? selectableRows.map(r => r.id) : [])}/></th><th>工单</th><th>客户</th><th>规格</th><th>待入库</th><th>在库数量</th><th title="本来源货批累计有效出库，不限查询日期；退货另行记录">已出数量</th><th>{view==='receipts' ? '本次入库' : view==='legacy' ? '历史结清' : view==='history' || filter==='shipped' ? '本次出库' : '本次处理'}</th><th>入库时间</th><th>出库时间</th><th>状态</th><th>操作</th></tr></thead>
        <tbody>{data.rows.map((row,index) => {
          ...
          return <Fragment key={row.id}>
            {groupStart && <tr className="fg-group"><td colSpan={12}><Layers3 size={13}/>{row.batchNumber ? `${row.batchNumber.slice(-2)} 批` : '未分批'}<span>{data.batches.find(b => b.id === row.batchId)?.name}</span></td></tr>}
            <tr data-fg-row={row.id} className={`${activeId === row.id ? 'active' : ''} ${selected.includes(row.id) ? 'selected' : ''}`} onClick={e => { if (!(e.target as HTMLElement).closest('input,button,select,a')) activate(row); }}>
              ...
              <td className="fg-spec-cell"><button className="fg-specification" onClick={() => open('detail',row)}>{row.specification || '未填写规格'}</button></td>
    ...
    {dialog && <div className={`fg-dialog-overlay ${dialog.kind==='detail' ? 'fg-detail-overlay' : ''}`}><section className={`fg-dialog ${dialog.kind==='detail' ? 'fg-detail' : ''}`} role="dialog" aria-modal="true" aria-label={titleFor[dialog.kind]}>
      ...
      {dialog.kind==='detail' && dialog.row && <>
        ...
        <div className="fg-detail-times"><span>完整工单 <b>{dialog.row.workOrderCode}</b><button className="fg-link" onClick={()=>void copyText(dialog.row!.workOrderCode)}><Copy size={13}/>复制</button></span><span>{dialog.row.status==='received' ? '本次入库' : '首次入库'} <b>{timeText(dialog.row.receivedAt)}</b></span>{dialog.row.receiptCount>1 && <span>最近入库 <b>{timeText(dialog.row.lastReceivedAt)}</b> · {dialog.row.receiptCount} 次</span>}{dialog.row.lastShippedAt && <span>{dialog.row.status==='shipped' ? '本次出库' : '最近出库'} <b>{timeText(dialog.row.status==='shipped' ? dialog.row.shippedAt : dialog.row.lastShippedAt)}</b></span>}<span>库位 <b>{dialog.row.location || '未设置'}</b></span></div>
