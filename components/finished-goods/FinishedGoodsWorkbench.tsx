...
import { Fragment, useCallback, useEffect, useRef, useState, type KeyboardEvent as ReactKeyboardEvent, type ClipboardEvent } from 'react';
...
import { FG_KINDS, fgCompactTime, fgShortWorkOrder, fgWaybills, type FgInput, type FgRow, type FgWorkbench } from '@/lib/finished-goods-domain';
import { canAccessAppRoute } from '@/lib/app-route-access';
import type { CurrentUserDTO } from '@/types';

type Draft = { quantity: string; waybills: string; method: string; batchId: string; note: string; externalReference: string };
...
const STATUS: Record<string, string> = { pending: '待入库', ready: '待发货', shipped: '已发货', held: '留库', opening: '待核对', legacy: '历史已出', received: '已入库', reserved: '已占用', blocked: '隔离', restricted: '生产受限' };
const VIEWS = [{ id: 'queue', label: '收发' }, { id: 'stock', label: '库存' }, { id: 'history', label: '记录' }, { id: 'batches', label: '批次' }];
const QUEUE_FILTERS = [{ id: 'processing', label: '待处理' }, { id: 'pending', label: '待入库' }, { id: 'ready', label: '待发货' }, { id: 'shipped', label: '已发货' }, { id: 'missing', label: '待补单号' }, { id: 'all', label: '全部' }];
...
function defaultDraft(row: FgRow): Draft { return { quantity: String(row.quantity || row.available || row.pending || 1), waybills: row.waybills.join('\n'), method: row.method, batchId: row.batchId, note: row.shipmentNote, externalReference: row.externalReference || '' }; }
...
export default function FinishedGoodsWorkbench({ user, initialData, initialQuery }: { user: CurrentUserDTO; initialData: FgWorkbench; initialQuery: string }) {
  const [data, setData] = useState(initialData); const [view, setView] = useState('queue'); const [filter, setFilter] = useState('processing');
  ...
  function pasteWaybills(event: ClipboardEvent<HTMLInputElement>, row: FgRow) { const text = event.clipboardData.getData('text'); if (/[\r\n]/.test(text)) { event.preventDefault(); updateDraft(row,'waybills',text.replace(/\r?\n/g,'；')); } }
  ...
  async function execute(input: FgInput, success: string, affected: FgRow[] = [], keepDialog = false): Promise<boolean> {
    ...
    try {
      ...
      clearDrafts(['RECEIVE','RECEIVE_HOLD','BATCH_RECEIVE'].includes(String(input.action)) ? affected.filter(row => { const draft = drafts[row.id]; return !draft || !(draft.waybills.trim() || draft.note.trim() || draft.externalReference?.trim() || draft.method !== workingMethod || draft.batchId !== workingBatchId); }) : affected);
      ...
      if (['SAVE_DRAFT','SAVE_LOGISTICS'].includes(String(input.action)) && affected.length === 1) { setSavedRowId(affected[0].id); setEditingWaybill(''); }
  ...
  function open(kind: string, row?: FgRow, rows?: FgRow[]) {
    ...
    setForm({ quantity: String(q || (kind === 'opening' ? 0 : 1)), reason: ['hold','receiveHold'].includes(kind) ? '备货' : '', location: row?.location || '', customerName: row?.customerName || '', ownerType: 'PUBLIC', bucket: row?.available ? 'available' : 'blocked', date: data.workDate, reworkId: '', waybills: '' });
  ...
  async function submitDialog() {
    ...
    if (dialog.kind === 'bulkWaybill') {
      let values: string[]; try { values = fgWaybills(form.waybills); } catch (error) { setError(error instanceof Error ? error.message : '单号格式错误'); return; }
      (dialog.rows || []).forEach(r=>updateDraft(r,'waybills',values.join('\n')));
      setDialog(null); setMessage('已填入所选行，保存或发货时生效'); return;
    }
  ...
  function waybillFields(row: FgRow) { const d = draftFor(row); return <>
    <label className="fg-span-2">快递单号<textarea rows={2} placeholder={d.method === 'COURIER' ? '输入或粘贴，多个单号换行；可先发后补' : '自提 / 送货无需填写'} value={d.waybills} onChange={e=>updateDraft(row,'waybills',e.target.value)}/></label>
    <details className="fg-extra-fields fg-span-2"><summary>备注 / 外部单号（可选）</summary><label>备注<input value={d.note} onChange={e=>updateDraft(row,'note',e.target.value)}/></label><label>关联外部单号<input placeholder="另一软件的出货单号或销货单号" value={d.externalReference} onChange={e=>updateDraft(row,'externalReference',e.target.value)}/></label></details>
  </>; }
  ...
  return <main className={`fg-shell hm-workbench-root hm-workbench-navigation-overlay ${size === 16 ? 'fg-comfortable' : 'fg-compact'} ${continuous ? 'fg-continuous' : ''}`}>
    ...
    <section className="fg-toolbar">
      ...
      {view !== 'legacy' && (view !== 'queue' || ['shipped','missing','all'].includes(filter)) && <label className="fg-date-filter">{view === 'stock' || view === 'receipts' ? '入库日期' : '出库日期'}<input aria-label={view === 'stock' || view === 'receipts' ? '入库日期' : '出库日期'} title="待处理包含往日未完成记录" type="date" value={date} onChange={e => { setDate(e.target.value); setScope(view === 'stock' ? 'range' : 'day'); setPage(1); setBatchId(''); }}/></label>}
      ...
      {!isRecords && view !== 'stock' && <><label className="fg-work-method">方式<select aria-label="本轮出货方式" value={workingMethod} onChange={e=>setWorkingMethod(e.target.value)}>{Object.entries(METHODS).map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></label><button type="button" className={continuous ? 'fg-toggle active' : 'fg-toggle'} onClick={() => { setContinuous(c => !c); if (!activeId) setActiveId(data.rows.find(canShip)?.id || ''); }}><ClipboardList size={15}/>连续处理</button></>}
      ...
      <div className="fg-more-wrap"><button aria-label="更多仓库操作" type="button" onClick={() => setMore(v => !v)}><MoreHorizontal size={19}/></button>{more && <div className="fg-menu">
        ...
        {!isRecords && <><button disabled={!selectedShipRows.length} onClick={applyWorkingBatch}>为所选设置本轮批次</button><button disabled={!selectedShipRows.length} onClick={()=>open('bulkWaybill',undefined,selectedShipRows)}>为所选统一填单号</button><button disabled={selectedShipRows.length < 2} onClick={() => open('merge', undefined, selectedShipRows)}><Layers3 size={14}/>合并待发记录</button><button onClick={() => open('stockAdd')}><Boxes size={14}/>登记实物库存</button></>}
    ...
    {view === 'batches' && <div className="fg-batch-strip">{data.batches.map(b => <div key={b.id} className={batchId === b.id ? 'active' : ''}><button onClick={() => { setBatchId(batchId === b.id ? '' : b.id); setPage(1); }}><strong>{b.sequence.toString().padStart(2,'0')} 批 {b.name}</strong><span>{b.shipped} 条已出 · {num(b.quantity)} 件 · {b.waybillCount} 个单号{b.missingWaybill > 0 ? ` · 待补 ${b.missingWaybill} 条` : ''}</span>{b.draft>0 && <small>{b.draft} 条待发，不计入实发</small>}</button>{b.closedAt ? <small>已封批</small> : <button className="fg-link" disabled={busy} onClick={() => void execute({ action:'CLOSE_BATCH', batchId:b.id },'该批次已封批')}>封批</button>}</div>)}<button className="fg-batch-new" onClick={() => open('batch')}><Plus size={16}/>新批次</button></div>}
    <section className="fg-table-scroll" aria-label="成品与出货明细" aria-busy={loading}>
      <table className="fg-table"><colgroup>{[30,112,100,112,84,70,118,118,156,96,126].map((width,i) => <col key={i} style={{width}}/>)}</colgroup>
        <thead><tr><th><input aria-label="全选本页可处理记录" type="checkbox" disabled={!selectableRows.length || busy || loading} checked={Boolean(selectableRows.length) && selectableRows.every(r => selected.includes(r.id))} onChange={e => setSelected(e.target.checked ? selectableRows.map(r => r.id) : [])}/></th><th>工单</th><th>客户</th><th>规格</th><th>{showStock ? '可用' : '数量'}</th><th>{showStock ? '留库' : view === 'receipts' ? '本次入库' : view === 'legacy' ? '结清数量' : '本次'}</th><th>入库时间</th><th>出库时间</th><th>{showStock ? '库位' : '快递单号'}</th><th>状态</th><th>操作</th></tr></thead>
        <tbody>{data.rows.map((row,index) => {
          ...
          const canEdit = !showStock && !readOnly && row.ownerType==='CUSTOMER' && (sent || actionable || row.status === 'reserved');
          ...
          return <Fragment key={row.id}>
            ...
            <tr data-fg-row={row.id} className={`${activeId === row.id ? 'active' : ''} ${selected.includes(row.id) ? 'selected' : ''}`} onClick={e => { if (!(e.target as HTMLElement).closest('input,button,select,a')) activate(row); }}>
              ...
              <td className="fg-stock-cell" title={`可用 ${row.available} · 待入 ${row.pending} · 占用 ${row.reserved} · 留库 ${row.held} · 隔离 ${row.blocked}`}>{readOnly || sent ? '—' : row.blockedReason ? '受限' : showStock ? row.available : row.status === 'pending' ? `待入 ${num(row.pending)}` : row.status === 'reserved' ? `占用 ${num(row.reserved)}` : `可发 ${num(row.available)}`}</td>
              <td>{showStock ? <span>{row.held || '—'}</span> : sent || readOnly ? <span className="fg-num">{row.quantity}{row.returned > 0 && <small title="已退回"> / 退{row.returned}</small>}</span> : !actionable && row.status !== 'reserved' ? '—' : <input type="number" aria-label={`${row.workOrderCode} 本次数量`} title={row.status === 'reserved' || row.shipmentLineCount > 1 ? '该单已占用或包含多条明细，请从合并记录整体处理' : 'Enter 保存，发货需单独确认'} min="1" className="fg-inline-number" value={d.quantity} readOnly={row.status === 'reserved' || row.shipmentLineCount > 1 || busy} onFocus={() => activate(row)} onChange={e => updateDraft(row,'quantity',e.target.value)} onKeyDown={e => { nextCell(e); if (e.key === 'Enter') { e.preventDefault(); void save(row); } if (e.key === 'Escape') resetField(row,'quantity'); }}/>}</td>
              <td className="fg-time" title={row.receivedAt ? `${row.status === 'received' ? '本次' : '首次'}入库 ${timeText(row.receivedAt)}` : legacy ? '历史入库时间未知' : '实物尚未入库'}>{row.receivedAt ? fgCompactTime(row.receivedAt) : legacy ? '未知' : '未入库'}</td>
              ...
              <td>{showStock ? <button className="fg-cell-text" title={row.location} onClick={() => open('move',row)}>{row.location || '设置库位'}</button> : <div className="fg-waybill">
                {canEdit ? <><input autoFocus={editingWaybill === row.id} aria-label={`${row.workOrderCode} 运单号`} title={d.waybills || 'Enter 保存单号，不会发货'} placeholder={d.method !== 'COURIER' ? METHODS[d.method] : sent ? '待补单号' : '填写单号'} readOnly={busy} value={d.waybills.replace(/\n/g,'；')} onFocus={()=>activate(row)} onChange={e=>updateDraft(row,'waybills',e.target.value)} onPaste={e=>{const text=e.clipboardData.getData('text');if(/[\r\n]/.test(text)){e.preventDefault();updateDraft(row,'waybills',text.replace(/\r?\n/g,'；'));}}} onKeyDown={e=>{nextCell(e);if(e.key==='Enter'){e.preventDefault();void save(row);}if(e.key==='Escape'){resetField(row,'waybills');setEditingWaybill('');}}}/>{drafts[row.id] ? <button title="保存信息，不会发货" aria-label={`保存 ${row.workOrderCode} 信息`} onClick={()=>void save(row)} disabled={busy || loading}><Check size={13}/></button> : row.waybills.length > 0 ? <button aria-label={`复制 ${row.workOrderCode} 运单`} title="复制全部运单" onClick={()=>void copyText(row.waybills.join('；'))}><Copy size={13}/></button> : savedRowId===row.id ? <Check className="fg-saved" size={13} aria-label="已保存"/> : null}</> : <span className="fg-cell-text" title={row.waybills.join('；')}>{legacy ? '未知' : row.waybills.length ? row.waybills.join('；') : '—'}</span>}
              </div>}</td>
              <td><button className={`fg-status ${row.status}`} title={sent && row.method==='COURIER' && !row.waybills.length ? '已发货，待补快递单号' : row.blockedReason || row.holdReason || STATUS[row.status]} onClick={() => open('detail',row)}><i/>{STATUS[row.status]}</button>{sent && row.method==='COURIER' && !row.waybills.length && <span className="fg-missing-dot" title="待补单号">待补</span>}{row.holdDueDate && row.holdDueDate <= data.workDate && row.held > 0 && <span className="fg-due" title="留库到期，需手动释放">到期</span>}</td>
              <td><div className="fg-actions">{sent || readOnly ? <button onClick={()=>open('detail',row)}>{sent && row.method==='COURIER' && !row.waybills.length ? '补单号' : '查看'}</button> : canReceive(row) && (filter==='pending' || showStock || row.ownerType==='PUBLIC') ? <button disabled={busy || loading} onClick={()=>open('receive',row)}>入库</button> : row.ownerType==='PUBLIC' && row.available>0 ? <button disabled={busy || loading} onClick={()=>open('allocate',row)}>分配客户</button> : row.status==='held' ? <button onClick={()=>open('release',row)}>释放留库</button> : row.status==='reserved' || actionable ? <button disabled={busy || loading} onClick={()=>open('ship',row)}>{row.status==='pending' ? '入库并发货' : '发货'}</button> : <button onClick={()=>open('detail',row)}>查看原因</button>}{!readOnly && <button aria-label={`${row.workOrderCode} 更多操作`} className="fg-row-more" onClick={()=>open('detail',row)}><MoreHorizontal size={14}/></button>}</div></td>
            </tr>
          </Fragment>;
        })}</tbody>
      </table>
    ...
    {continuous && <section className="fg-dock" aria-label="连续处理">{active && current && ['ready','pending','reserved'].includes(active.status) ? <>
      ...
      <label className="fg-dock-waybill">运单号<input placeholder="输入或粘贴单号" value={current.waybills.replace(/\n/g,'；')} onPaste={e=>pasteWaybills(e,active)} onChange={e => updateDraft(active,'waybills',e.target.value)} onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); void save(active); } }}/></label>
      ...
      <button className="fg-primary" disabled={busy || loading || active.ownerType==='PUBLIC'} onClick={()=>active.shipmentLineCount>1 ? open('ship',active) : void dispatch(active)}>{busy ? '正在处理…' : active.status==='pending' ? '确认入库并发货 · 下一条' : '确认发货 · 下一条'}<ArrowRight size={16}/></button>
    ...
    {dialog && <div className={`fg-dialog-overlay ${dialog.kind==='detail' ? 'fg-detail-overlay' : ''}`}><section className={`fg-dialog ${dialog.kind==='detail' ? 'fg-detail' : ''}`} role="dialog" aria-modal="true" aria-label={titleFor[dialog.kind]}>
      ...
      {dialog.row && <div className="fg-dialog-identity"><strong>{dialog.row.specification || '未填写规格'}</strong><span>{dialog.row.customerName || '公共备货'} · {STATUS[dialog.row.status]}</span><small>{dialog.row.productName}</small><div className="fg-stock-summary"><span>待入 <b>{num(dialog.row.pending)}</b></span><span>可用 <b>{num(dialog.row.available)}</b></span>{dialog.row.held>0 && <span>留库 <b>{num(dialog.row.held)}</b></span>}{dialog.row.reserved>0 && <span>占用 <b>{num(dialog.row.reserved)}</b></span>}{dialog.row.blocked>0 && <span>隔离 <b>{num(dialog.row.blocked)}</b></span>}</div></div>}
      {dialog.kind==='ship' && dialog.row && <>
        <p className="fg-ship-explanation">{Number(draftFor(dialog.row).quantity)>dialog.row.available && dialog.row.status!=='reserved' ? `本次将入库并发出差额 ${Math.max(0,Number(draftFor(dialog.row).quantity)-dialog.row.available)} 件；其余待入数量保持待入库。` : '确认后记录实际出库时间，未发出的库存继续保留。'}</p>
      ...
      {['batchShip','batchReceive'].includes(dialog.kind) && <>
        <p>本次 {dialog.rows?.length || 0} 条，共 {num((dialog.rows || []).reduce((sum,r)=>sum+Number(draftFor(r).quantity || 0),0))} 件。{dialog.kind==='batchShip' ? '各条保留自己的数量与单号。' : '只登记实物入库，不发货。'}</p>
        {dialog.rows?.map(r=>{const problem=entryProblem(r,dialog.kind==='batchReceive');return <div className={`fg-bulk-entry ${problem ? 'invalid' : ''}`} key={r.id}><div><strong>{fgShortWorkOrder(r.workOrderCode)} · {r.customerName || '公共备货'}</strong><small>{r.specification}{problem ? ` · ${problem}` : ''}</small>{dialog.kind==='batchShip' && <input className="fg-bulk-waybill" aria-label={`${r.workOrderCode} 批量单号`} placeholder="填写单号（可后补）" value={draftFor(r).waybills.replace(/\n/g,'；')} onPaste={e=>pasteWaybills(e,r)} onChange={e=>updateDraft(r,'waybills',e.target.value)}/>}</div><label>数量<input aria-label={`${r.workOrderCode} 批量数量`} type="number" min="1" value={draftFor(r).quantity} onChange={e=>updateDraft(r,'quantity',e.target.value)}/></label><button aria-label={`移出 ${r.workOrderCode}`} onClick={()=>setDialog(prev=>prev ? {...prev,rows:prev.rows?.filter(item=>item.id!==r.id)} : null)}><X size={14}/></button></div>;})}
        {bulkInvalid && <p className="fg-field-error">请修正数量或移出不符合条件的记录后再提交。</p>}
      </>}
      {dialog.kind==='bulkWaybill' && <><p>填入已选 {dialog.rows?.length || 0} 条待发记录，保存或发货时生效。</p>{new Set(dialog.rows?.map(r=>r.customerName)).size>1 && <p className="fg-callout">所选包含不同客户，请确认这些货确实共用运单。</p>}<div className="fg-form-grid"><label className="fg-span-2">统一快递单号<textarea rows={3} value={form.waybills || ''} onChange={e=>setForm(f=>({...f,waybills:e.target.value}))} placeholder="多个单号换行"/></label></div><p className="fg-muted">{dialog.rows?.map(r=>`${fgShortWorkOrder(r.workOrderCode)} · ${r.customerName}`).join('；')}</p></>}
      {dialog.kind==='merge' && <><p>仅同客户可合并；使用第一条的单号、出货方式和批次。已有待发记录需先取消，库存占用会同步释放。</p>{dialog.rows?.map(r=><div className="fg-bulk-entry" key={r.id}><strong>{fgShortWorkOrder(r.workOrderCode)} · {r.customerName}</strong><span>{draftFor(r).quantity} {r.unit}</span></div>)}</>}
      {!['ship','batchShip','batchReceive','bulkWaybill','detail','merge'].includes(dialog.kind) && <div className="fg-form-grid">
      ...
      {dialog.kind==='detail' && dialog.row && <>
        ...
        <div className="fg-detail-times"><span>完整工单 <b>{dialog.row.workOrderCode}</b><button className="fg-link" onClick={()=>void copyText(dialog.row!.workOrderCode)}><Copy size={13}/>复制</button></span><span>{dialog.row.status==='received' ? '本次入库' : '首次入库'} <b>{timeText(dialog.row.receivedAt)}</b></span>{dialog.row.shippedAt && <span>本次出库 <b>{timeText(dialog.row.shippedAt)}</b></span>}</div>
        {dialog.row.status==='legacy' && <p className="fg-callout">启用前已默认结清，结清于 {timeText(dialog.row.legacyClosedAt)}；实际出库时间和运单未知。</p>}
        {dialog.row.blockedReason && <p className="fg-callout"><AlertCircle size={15}/>{dialog.row.blockedReason}，处理来源后才能发货。</p>}
        {(!dialog.row.legacyClosedAt || dialog.row.status==='shipped') && dialog.row.ownerType==='CUSTOMER' && (dialog.row.shipmentId || isActionable(dialog.row)) && <section className="fg-detail-section"><h3>{dialog.row.status==='shipped' ? '本次出库' : '待发信息'}</h3>{dialog.row.shipmentId && <p className="fg-record-summary">{dialog.row.shipmentNumber} · {dialog.row.quantity} {dialog.row.unit} · {METHODS[draftFor(dialog.row).method]}</p>}<div className="fg-form-grid">{waybillFields(dialog.row)}</div><p className="fg-muted">{dialog.row.status==='shipped' ? '补单号不改变出库时间和库存。' : '保存信息不扣库存，发货需单独确认。'}</p></section>}
        ...
        <section className="fg-detail-section"><h3><History size={15}/>收发流水</h3>{!detail ? <p className="fg-muted">正在加载…</p> : detail.lot.ledger.length ? <ol className="fg-ledger">{detail.lot.ledger.map(l=><li key={l.id}><span className="fg-ledger-dot"/><div><strong>{FG_KINDS[l.kind] || (l.kind==='LOGISTICS' ? '单号与备注更新' : l.kind)} <b>{l.quantity ? `${['ADJUST','SOURCE_CORRECTION'].includes(l.kind) ? l.quantity : Math.abs(l.quantity)} 件` : ''}</b></strong>{l.reason && <p>{l.reason}</p>}<small>{timeText(l.createdAt)} · {l.actorName}</small></div></li>)}</ol> : <p className="fg-muted">尚无实物收发记录。</p>}</section>
        {Boolean(detail?.lot.lines.length) && <details className="fg-extra-fields fg-detail-section"><summary>关联出库记录</summary>{detail?.lot.lines.map(l=><div className="fg-related-record" key={l.id}><strong>{l.shipment.number}</strong><span>{l.quantity} 件 · {l.shipment.status==='SHIPPED' ? '已出库' : l.shipment.status==='CANCELLED' ? '已取消' : '待发货'}{l.returned>0 && ` · 退回 ${l.returned}`}</span><small>{timeText(l.shipment.shippedAt)}</small></div>)}<button className="fg-link" onClick={()=>{switchView('history');setScope('all');setSearch(dialog.row!.workOrderCode);setDialog(null);}}>查询该工单全部出库记录<ArrowRight size={13}/></button></details>}
