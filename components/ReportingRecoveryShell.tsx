...
export default function ReportingRecoveryShell({ user }: { user: CurrentUserDTO }) {
  ...
  const loadPreview = useCallback(async (id: string) => {
    ...
    try {
      ...
      const options = next.sourceOptions.filter(option => option.action !== 'FUTURE_CONFIRMATION' && option.action !== 'HISTORICAL_CONFIRMATION'
        && option.remainingQty >= Math.max(1, next.submission.processedQty - next.submission.defectQty));
  ...
  const sourceSufficient = !needsSource || Boolean(source && source.remainingQty >= Math.max(1, Number(processedQty) - Number(defectQty)));
  const canConfirm = Boolean(preview?.canResolve && submission?.status === 'PENDING' && !previewLoading && !saving && !preview.blockers.length && mappingValid && standardValid && sourceSufficient && (!needsSource || source) && (!source || !['FUTURE_CONFIRMATION', 'HISTORICAL_CONFIRMATION'].includes(source.action) || advanceConfirmed));
