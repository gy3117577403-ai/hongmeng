...
export default function EmployeeQualityWarning({ warning }: { warning: View }) {
  ...
  const fields = [['本批必须注意', warning.requiredAction], ['问题是什么', warning.defectPhenomenon || warning.warningSummary], ['已确认的原因', warning.rootCause || warning.occurrenceCause], ['具体解决方案', warning.correctiveAction], ['临时遏制', warning.containmentAction], ['防止再发', warning.preventiveAction], ['处理结论', warning.finalConclusion], ['检查方法', warning.inspectionMethod], ['检查频次', warning.inspectionFrequency], ['合格判定', warning.acceptanceCriteria], ['停止作业 / 升级条件', warning.stopConditions]].filter(([, value]) => value);
  return <main className="employee-warning"><header><ShieldAlert /><div><small>员工只读 · 已发布归档</small><h1>{qualityEventTitle(warning)}</h1><p>{warning.reportNo} · <b>R{warning.revisionNumber}</b></p></div></header>
    ...
    <nav className="employee-anchor" aria-label="现场作业指引"><a href="#employee-solution">问题与方案</a><a href="#employee-photos">完整图片 ({photos.length})</a></nav>
