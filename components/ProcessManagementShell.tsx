...
export default function ProcessManagementShell({ user }: { user: CurrentUserDTO }) {
  ...
  function moveStep(index: number, direction: -1 | 1): void {
    setDraftSteps(current => {
      ...
      return replaceStepPositions(copy);
    });
  }

  function removeStep(index: number): void {
    setDraftSteps(current => replaceStepPositions(current.filter((_, itemIndex) => itemIndex !== index)));
  }

  function addDefinition(definition: ProcessDefinitionDTO): void {
    setDraftSteps(current => {
      if (current.some(step => step.processCode === definition.code)) {
  ...
  function addShortcut(group: ShortcutGroup): void {
    ...
    setDraftSteps(current => {
      ...
      if (!additions.length) setToast(`${group.name} 已完整加入`);
  ...
  function addCustomStep(): void {
    ...
    setDraftSteps(current => replaceStepPositions([...current, {
      clientId: code,
  ...
  async function confirmRoute(): Promise<void> {
    ...
    try {
      const response = await fetch(`/api/process-management/routes/${routeDetail.id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'confirm', version: routeDetail.version }),
      });
      const body = await response.json().catch(() => ({})) as {
        route?: WorkOrderProcessRouteDTO;
        error?: string;
    ...
    <main ref={mainRef} className="process-workbench hm-workbench-root">
      ...
      <div className="process-page-frame">
        ...
        <section className="process-toolbar" aria-label="工艺路线筛选">
          ...
          <button className="process-library-toggle" type="button" aria-expanded={libraryOpen} onClick={() => setLibraryOpen(value => !value)}>{libraryOpen ? <PanelRightClose size={16} aria-hidden="true" /> : <PanelRightOpen size={16} aria-hidden="true" />}工序库</button>
        </section>
        ...
        <div className={`process-main-grid ${libraryOpen ? 'library-open' : ''}`}>
          <section className="process-order-panel" aria-labelledby="process-order-heading">
            <header><div><span>{scope === 'current' ? '当前生产周' : '历史生产周'}</span><h2 id="process-order-heading">待编排工单</h2></div><em>{payload?.orders.length || 0} 项</em></header>
            ...
            {selectedOrder ? <>
              ...
              {!routeLoading && routeDetail && <div className="process-route-body">
                ...
                <div className="process-step-list hm-scroll-region" tabIndex={0} aria-label="工艺路线步骤">
                  {(editable ? draftSteps : routeDetail.steps).map((step, index) => <article className={`process-step-row ${'status' in step ? step.status : 'pending'} ${step.stageGroup}`} key={'clientId' in step ? step.clientId : step.id}>
                    <div className="process-step-index"><GripVertical size={15} aria-hidden="true" /><b>{String(index + 1).padStart(2, '0')}</b></div>
                    <div className="process-step-name"><strong>{step.processName}</strong><span>{groupText[step.stageGroup]}</span></div>
                    {'status' in step && <span className={`process-step-status ${step.status}`}>{step.status === 'current' ? '当前' : step.status === 'completed' ? '完成' : step.status === 'skipped' ? '跳过' : '待开始'}</span>}
                    {editable && <div className="process-step-actions"><button type="button" disabled={index === 0} aria-label={`上移${step.processName}`} title="上移" onClick={() => moveStep(index, -1)}><ArrowUp aria-hidden="true" /></button><button type="button" disabled={index === draftSteps.length - 1} aria-label={`下移${step.processName}`} title="下移" onClick={() => moveStep(index, 1)}><ArrowDown aria-hidden="true" /></button><button className="danger" type="button" aria-label={`删除${step.processName}`} title="删除工序" onClick={() => removeStep(index)}><Trash2 aria-hidden="true" /></button></div>}
                    {'completedAt' in step && step.completedAt && <small className="process-step-time">{dateTimeText(step.completedAt)}</small>}
                  </article>)}
                </div>
                ...
                <footer className="process-route-actions">
                  {editable ? <><button type="button" disabled={saving} onClick={() => setLibraryOpen(true)}><PackagePlus size={16} aria-hidden="true" />从工序库添加</button><button type="button" disabled={saving} onClick={() => { void saveDraft(); }}><Save size={16} aria-hidden="true" />保存草稿</button><button className="primary-button" type="button" disabled={saving || draftSteps.length === 0} onClick={() => { void confirmRoute(); }}><ClipboardCheck size={16} aria-hidden="true" />确认路线</button></> : <><a className="hm-workbench-button" href="/production">前往生产执行</a><span>已确认路线锁定，模板更新不会影响本工单。</span></>}
                </footer>
          ...
          <aside className={`process-library-panel ${libraryOpen ? 'open' : ''}`} aria-label="工序库">
            <header><div><span>快捷编排</span><h2>工序库</h2></div><button type="button" aria-label="关闭工序库" title="关闭" onClick={() => setLibraryOpen(false)}><X aria-hidden="true" /></button></header>
            <div className="process-library-body hm-scroll-region">
              ...
              <section><h3>标准工序</h3><div className="process-definition-list">{payload?.definitions.map(definition => <button type="button" key={definition.id} disabled={!editable || draftSteps.some(step => step.processCode === definition.code)} onClick={() => addDefinition(definition)}><span><strong>{definition.name}</strong><small>{groupText[definition.stageGroup]}</small></span><Plus size={15} aria-hidden="true" /></button>)}</div></section>
    ...
    {templateOpen && <>
      ...
      <aside ref={drawerRef} className="process-template-drawer" role="dialog" aria-modal="true" aria-labelledby="process-template-title">
        ...
        <div className="process-template-body hm-scroll-region">
          ...
          <section><h3>模板工序顺序</h3><div className="process-template-steps">{templateSteps.map((step, index) => <article key={step.clientId}><b>{index + 1}</b><span><strong>{step.processName}</strong><small>{groupText[step.stageGroup]}</small></span><button type="button" disabled={index === 0} aria-label={`上移${step.processName}`} title="上移" onClick={() => setTemplateSteps(current => { const copy = [...current]; const item = copy[index]; copy[index] = copy[index - 1]; copy[index - 1] = item; return replaceStepPositions(copy); })}><ArrowUp aria-hidden="true" /></button><button type="button" disabled={index === templateSteps.length - 1} aria-label={`下移${step.processName}`} title="下移" onClick={() => setTemplateSteps(current => { const copy = [...current]; const item = copy[index]; copy[index] = copy[index + 1]; copy[index + 1] = item; return replaceStepPositions(copy); })}><ArrowDown aria-hidden="true" /></button><button className="danger" type="button" aria-label={`删除${step.processName}`} title="删除工序" onClick={() => setTemplateSteps(current => replaceStepPositions(current.filter((_, itemIndex) => itemIndex !== index)))}><Trash2 aria-hidden="true" /></button></article>)}</div></section>
