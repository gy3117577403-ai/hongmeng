...
import ReportQualityEditor from './ReportQualityEditor';
...
export default function QualityDataDetail({ record, user, onChanged, onEdit, onRetest }: { record: QualityRecord; user: CurrentUserDTO; onChanged: (r: QualityRecord) => void; onEdit: () => void; onRetest: () => void }) {
  ...
  return <section className="qd-detail" aria-label="检验记录详情">
    ...
    <div className="qd-table-scroll"><table className="qd-measure-table"><thead><tr><th>样本 / 位置</th><th>项目与标准</th><th>实测结果</th><th>判定</th></tr></thead><tbody>{r.data.rows.map((row,i) => <tr key={i}><td>{row.sample || '—'}<small>{row.position}</small></td><td><b>{row.item}</b><small>{row.standard || '未填写文字依据'}{(row.lower || row.upper) && ' · ' + (row.lower || '不限') + ' ～ ' + (row.upper || '不限') + ' ' + row.unit}</small>{row.note && <small>{row.note}</small>}</td><td>{row.value || '—'} {row.unit}</td><td><span className={'qd-badge ' + row.result.toLowerCase()}>{RESULT_LABELS[row.result]}</span></td></tr>)}</tbody></table>{!r.data.rows.length && <p className="qd-help">{r.sourceCompletionId ? '本条记录检验数量与不良情况；实测参数请通过独立检验或复检记录填写。' : '文件归档记录，请查看附件与内容摘要。'}</p>}</div>
    ...
    <div className="qd-detail-actions">
      {!old && canEdit && <button onClick={() => record.sourceCompletionId ? setReportEditor(true) : onEdit()} disabled={busy}><Pencil size={15}/>{record.sourceCompletionId ? '补充责任与说明' : '编辑 / 修订'}</button>}
    ...
    {reportEditor && <ReportQualityEditor record={record} onSaved={onChanged} onClose={() => setReportEditor(false)}/>}
