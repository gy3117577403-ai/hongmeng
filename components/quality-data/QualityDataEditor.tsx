...
export default function QualityDataEditor({ user, order, type, record, sourceQrCode, supersedesId, onSaved, onClose }: Props) {
  ...
  return <section className="qd-editor qd-editor-compact" aria-label={QUALITY_LABELS[type]+'填报'}>
    ...
    <div className="qd-editor-body">
      ...
      <section className="qd-inspection-focus"><b>{type === 'CRIMP' ? '端子压检 · 外观检查' : type === 'PULL' ? '拉力测试 · 端子型号与拉力' : type === 'FINAL' ? '成品检验 · 外观与尺寸' : type === 'CONTINUITY' ? '导通检验 · 电气连接检查' : '本次抽检数量'}</b><div className="qd-context-grid">{CONTEXT_FIELDS.filter(([name]) => ['inspectionQty','sampleQty','defectQty', ...(type === 'PULL' ? ['terminal','wire'] : [])].includes(name)).map(([name,label]) => <label key={name}>{label}<input aria-label={label} inputMode={['inspectionQty','sampleQty','defectQty'].includes(name) ? 'numeric' : undefined} value={form.context[name]} onChange={e => change({ ...form, context: { ...form.context, [name]: e.target.value } })}/></label>)}</div></section>
