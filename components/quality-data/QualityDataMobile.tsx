...
import { QUALITY_SCAN_TYPES, QUALITY_LABELS, RESULT_LABELS, beijingInput, type QualityOrder, type QualityRecord, type QualityDataType } from '@/lib/quality-data';
...
export default function QualityDataMobile({ code,user }: { code: string; user: CurrentUserDTO; initialType?: string; initialStepId?: string }) {
  ...
  useEffect(()=>{
    ...
    qualityRequest<QualityOrder>('qr/'+encodeURIComponent(code)).then(async value=>{
      ...
      const records=await qualityRequest<{items:QualityRecord[]}>('records?period=all&scan=1&workOrderId='+encodeURIComponent(value.id));
