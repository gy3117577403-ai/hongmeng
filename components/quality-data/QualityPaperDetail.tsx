...
import { ChevronLeft, ChevronRight, ClipboardCheck, Download, FileText, History, MoreHorizontal, Pencil, RotateCcw, Trash2, X } from 'lucide-react';
...
export default function QualityPaperDetail({ record, user, onChanged, onEdit }: { record: QualityRecord; user: CurrentUserDTO; onChanged: (record: QualityRecord) => void; onEdit: () => void }) {
  ...
  return <section className="qp-detail" aria-label="检验照片预览">
    <header><div><small>{QUALITY_LABELS[record.type]}{record.inspectionStepSnapshot ? ' › 第 ' + String(record.inspectionStepSnapshot.position || '').padStart(2, '0') + ' 道 · ' + record.inspectionStepSnapshot.name : first ? ' · 历史未指定工序' : ''}</small><h2>{first ? record.orderSnapshot.specification || record.orderSnapshot.productName : record.title}</h2><span>{first ? (record.orderSnapshot.businessCode || record.orderSnapshot.code) + (record.orderSnapshot.batchNo ? ' · 第 ' + record.orderSnapshot.batchNo + ' 批' : '') : record.data.paper?.area || '纸质巡检记录'}</span></div><div className="qp-detail-actions"><span className={'qd-badge ' + record.result.toLowerCase()}>{record.deletedAt ? '已作废' : record.status === 'DRAFT' ? '草稿' : first ? RESULT_LABELS[record.result] : '已归档'}</span><details><summary aria-label="更多记录操作"><MoreHorizontal size={20}/></summary><div>{canEdit && <button onClick={() => requestPreviewLeave(onEdit)}><Pencil size={15}/>编辑 / 补传照片</button>}<button onClick={() => setHistory(true)}><History size={15}/>原始明细与历史</button>{!record.deletedAt && (canManage || (record.createdById === user.id && record.status === 'DRAFT')) && <button disabled={busy} onClick={() => {setPendingAction('DELETE');setActionReason('');}}><Trash2 size={15}/>移入回收站</button>}{record.deletedAt && canManage && <button disabled={busy} onClick={() => {setPendingAction('RESTORE');setActionReason('');}}><RotateCcw size={15}/>恢复记录</button>}</div></details></div></header>
    <div className="qp-detail-meta"><div><small>{first ? '实际检验' : '巡检日期'}</small><b>{first ? beijingInput(record.inspectedAt).replace('T', ' ') : beijingInput(record.inspectedAt).slice(0, 10)}</b></div><div><small>检验人</small><b>{record.data.context.inspectedBy || record.createdByName}</b></div><div><small>检验凭证</small><b>{files.length} 份照片 / 文件</b></div></div>
