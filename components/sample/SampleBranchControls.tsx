...
export function SampleBranchControls({ type, week, carry, refresh, onChange, warehouse = false }: { warehouse?: boolean; type: 'NEW' | 'REPEAT' | ''; week: string; carry: boolean; refresh: number; onChange: (type: 'NEW' | 'REPEAT' | '', week: string, carry: boolean) => void }) {
  ...
  return <section className="sb-plan-nav" aria-label="样品类型与计划周">
    {warehouse ? <select aria-label="样品类型" value={type} onChange={e=>onChange(e.target.value as 'NEW'|'REPEAT'|'',week,carry)}><option value="">全部样品</option><option value="NEW">新品试制</option><option value="REPEAT">老产品制作</option></select> : <div className="sb-branch-switch" role="group" aria-label="样品类型">{(['NEW','REPEAT'] as const).map(kind => <button key={kind} className={type === kind ? 'active' : ''} aria-pressed={type === kind} onClick={() => onChange(kind, week, carry)}>{kind === 'NEW' ? <FlaskConical size={19}/> : <Layers3 size={19}/>}<span>{kind === 'NEW' ? '新品试制' : '老产品制作'}</span></button>)}</div>}
