...
3. App Deploy 候选镜像：`ghcr.io/gy3117577403-ai/hongmeng:v1.31.3`。生产环境当前实际版本需在切换前现场核验；候选镜像完成隔离验收后，应固定到其不可变 digest，不使用 `latest` 切换。
4. 端口：`3000`。
5. 环境变量：
```env
...
- `v1.31.3` 重构“变更管理”为高级三栏工作台：去除重复标题，增加范围页签、队列分组、完整工艺路线前后对比、影响责任栏、固定协作区和工艺审核精确入口；不新增数据库迁移。
