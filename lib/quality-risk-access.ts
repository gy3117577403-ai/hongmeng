...
export async function requireQualityRiskParticipant(reportId: string, mode: 'read' | 'manage' | 'task' = 'read') {
  ...
  const report = await prisma.internalQualityRiskReport.findUnique({ where: { id: reportId }, select: { ownerUserId: true, createdById: true, deletedAt: true, tasks: { where: { ownerUserId: user.id }, select: { id: true } } } });
