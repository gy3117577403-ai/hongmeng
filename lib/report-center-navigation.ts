...
export const REPORT_DOMAINS: readonly ReportDomainDefinition[] = [
  ...
  },
  {
    ...
    branches: [
      ...
      { key: 'employee-attainment', label: '员工工时达成', shortLabel: '员工工时达成', description: '按日、周、月汇总个人出勤、完成工时、已确认损耗与其他工时，查看报工明细' },
      { key: 'team-hours', label: '班组工时达成', shortLabel: '班组工时', description: '按班组汇总正常出勤、实际加班、完成工时、已确认损耗与其他工时' },
...
export function legacyReportRoute(
  ...
  if (firstValue(searchParams.branch) === 'unmatched-labor') return reportRoute('people', 'employee-attainment');
