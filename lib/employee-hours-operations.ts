  ...
  return rows.map(row => {
    ...
    const days: ReportOperationsEmployeeDayDTO[] = dateKeys.map(date => {
      ...
      return {
        ...
        pendingMatchingMilliseconds: day?.pendingMatchingMilliseconds ?? 0,
        pendingReviewMilliseconds: day?.pendingReviewMilliseconds ?? 0,
        reportedDurationMilliseconds: day?.reportedDurationMilliseconds ?? 0,
        missingTimeRecordCount: day?.missingTimeRecordCount ?? 0,
        scheduledTargetMilliseconds: day?.scheduledTargetMilliseconds ?? 0,
        estimatedCapacityMilliseconds: day?.estimatedCapacityMilliseconds ?? 0,
        estimatedAttainmentBasisPoints: day?.estimatedAttainmentBasisPoints ?? null,
