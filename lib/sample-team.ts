...
export function serializeSampleTask(task: SampleTaskRecord): SampleTaskDTO {
  ...
  return {
    ...
    issuedDate: task.issuedDate?.toISOString().slice(0, 10) || null,
    warningDays: task.warningDays,
    scheduleHistory: (Array.isArray(task.scheduleHistory) ? task.scheduleHistory : []) as import('@/types').SampleScheduleChange[],
