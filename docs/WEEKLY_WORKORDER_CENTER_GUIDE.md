...
从 v1.14.7 开始，周计划不再允许导入后直接覆盖当前周。确认导入只生成下周草稿，并自动进入周计划差异中心；处理阻断异常后才能启用。
...
启用预检会返回新增、延续、变更、下周取消、阻断异常、警告以及有 / 无图纸资料数量。阻断异常大于 0 时 commit 返回 `409`，不执行周切换。警告可以继续，但确认弹窗会列出数量。

完整差异审核规则见 [周计划差异中心使用说明](WEEKLY_PLAN_DIFF_CENTER_GUIDE.md)。
...
当前周在启用后进入 `/production` 生产执行中心；下周草稿和历史周不会进入现场看板。生产执行中心负责负责人、工位、完成数量、状态与进度备注，周计划中心继续负责导入、归档和周切换。
