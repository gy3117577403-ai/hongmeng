...
v1.14.5 起，网页端批量导入和命令行批量导入共用同一套有效规格规则：

- 纯日期不会作为规格，例如 `2026-03-05`、`2026/03/05`、`2026.03.05`。
- 日期时间不会作为规格，例如 `2026-03-05 10.17` 或 `2026-03-05 10:17`。
- 文件名只识别到日期时，预览状态为“规格无法识别”，原因显示“文件名只识别到日期，不是有效规格”。
- 这类文件会进入 `unmatched.csv`，不会自动创建图纸资料记录，也不会上传到 S3。
- 如确实需要导入，先重命名为真实产品规格，或在后续人工修正规格后再上传。
