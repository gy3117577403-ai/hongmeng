...
v1.14.4 起，图纸资料库首页不再反复展示缺失分类明细。客户列表、规格卡片和详情标题只保留资料完整度，例如 `资料 1/5`，不再显示 `缺 SOP`、`缺成品图` 这类长文案。具体每类是否已有文件，可通过分类栏数量判断。
...
v1.14.3 起，图纸资料库接入生产工单页顶部全局搜索。

- 在生产工单页搜索规格、客户、品名或图纸文件名时，搜索浮层会分组显示“图纸资料”和“图纸文件”。
- 点击图纸资料结果会跳转到 `/drawing-library` 并自动选中对应客户和规格。
- 点击图纸文件结果会跳转到 `/drawing-library` 并自动选中所属规格、分类和文件。
- 生产工单列表仍然只显示生产工单，图纸资料库记录不会伪装成工单。
- 图纸资料库自身搜索框仍聚焦长期图纸资料，可按规格、客户、品名、备注和图纸文件名过滤。
...
图纸资料库重排为“客户 / 规格 / 图纸预览”的三栏工作台：

- 左侧为紧凑客户列表和规格列表，规格作为主标题，客户和品名作为副标题。
- 中间为主预览区，分类栏收在预览区左侧，PDF / 图片默认尽量占满可用空间。
- 右侧只显示当前分类文件列表，下载和删除操作集中在文件面板内。
- 详情标题区只保留规格、客户、品名、资料数和更新时间，不再重复显示大字段卡片。
- 同屏只保留一个关键空态，避免“暂无文件 / 空资料”重复出现。

异常规格不会默认显示在正常列表里。进入“异常数据”筛选后，可查看日期型规格、空规格、无文件空记录或未归档记录，并通过编辑修正规格。

日期型文件名不会自动生成规格。例如 `2026-03-05.pdf`、`2026-03-05 10.17.pdf` 或 `线束图纸2026-03-05 10.17.pdf` 会进入未匹配，不会自动创建 `2026-03-05` 规格。
