...
- 周计划导入后会自动创建或关联长期图纸资料库，但图纸资料库不显示周计划状态字段。
...
8. 打开“图纸资料库”，确认导入行按客户和规格生成长期资料记录。
9. 确认图纸资料库记录只包含客户、规格、品名、备注和资料文件，不包含周计划生产状态字段。

## v1.13.6 周计划显示与本周清理预检

1. 导入周计划后，确认工单卡片主标题显示 Excel “规格”，例如 `D010304-8233-V02`。
2. 确认当前工单信息条、搜索结果、打印摘要、二维码标题和 ZIP 文件名都优先显示规格 / 生产编号。
3. 确认仍可用内部 `WO-*` 编号搜索到工单，但页面不把它作为主标题展示。
4. 在系统设置中选择同一计划周开始日期，点击“预览清理”。
5. 确认预览显示本周生产工单数量、已上传资料工单数量、不会删除的文件数量、不会删除的连接器参数数量。
6. 不输入 `CLEAR_WEEK` 时确认不能执行正式清理。
7. 输入 `CLEAR_WEEK` 后执行清理，确认默认工单列表不再显示本周已清理生产工单。
8. 清理后确认已上传 PDF / 图片仍保留，连接器参数页面不受影响。
9. 该清理不同于 `scripts/clear-workorder-data.mjs`，不做物理删除，不删除 S3 对象。
10. 清理后确认图纸资料库记录和图纸资料文件仍保留，后续同规格周计划可继续关联。
