...
1. 确认当前启用周摘要和四状态数量与数据库一致。
2. 分别验证今日交期、逾期、未发图、配料未齐和资料不完整。
3. 用测试工单验证开工、完成、撤回完成与进度历史。
4. 批量完成必须验证错误确认词不写入、`COMPLETE_BATCH` 才允许执行。
5. 确认下周草稿和历史周不会进入执行看板。
6. 在 1024x768、1280x800、1366x1024 检查看板、弹窗和 Portal 菜单。
7. 回归周计划、连接器参数、PDF、图片、上传、预览和下载。
