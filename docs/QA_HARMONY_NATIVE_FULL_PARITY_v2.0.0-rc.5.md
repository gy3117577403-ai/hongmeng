...
| 工单搜索 | 是 | 是 | 是 | 抽屉搜索按工单过滤；顶部全局搜索调用 `/api/native/search` 并展示工单/文件结果。 |
...
| 复制工单链接 / 二维码 | 是 | 部分 | 部分 | 工单链接已接入 `ClipboardAdapter`；二维码渲染未显示空按钮，后续可接专用二维码组件。 |
...
| 拍照上传 | 是 | 部分 | 需真机验证 | `CameraAdapter` 先接系统图片选择器作为拍照/相册入口；如设备未提供相机入口，提示先拍照后选择图片。 |
...
| 原始资料附件下载 | 是 | 是 | 需真机验证 | 下载 URL 已接入，系统保存/打开由 `DownloadAdapter` 承接。 |
...
| 全局搜索 | 是 | 是 | 是 | 顶部搜索调用 native search，结果面板可直达工单或定位文件。 |
...
## 受限能力说明
- 原生文件选择、相机入口、系统级保存/打开、剪贴板和语音输入继续通过 platform adapter 承接。
