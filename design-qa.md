...
## 2026-08-21 问题五阶段关闭、管理员代办与完结问题库

### 验收对象

- 用户参考图：`C:\Windows\TEMP\codex-clipboard-bc36bb82-0bb6-4b30-8b3a-7c1ff50a7f83.png`。
- 验收视口：1366 × 1024 CSS px，截图密度 1×。
- 待发起人确认：`artifacts/issue-closure-v13433/awaiting-confirmation-1366x1024.png`。
- 管理员确认完结弹窗：`artifacts/issue-closure-v13433/admin-confirm-modal-1366x1024.png`。
- 文件与证据分支：`artifacts/issue-closure-v13433/file-evidence-branch-1366x1024.png`。
- 已完结问题库筛选：`artifacts/issue-closure-v13433/closed-archive-filter-1366x1024.png`。

### 流程与权限验收

- 生命周期使用“待受理 → 处理中 → 待验证 → 待发起人确认 → 已关闭”五阶段；右侧进度与顶部状态筛选采用同一状态源。
- 验证人通过后进入“待发起人确认”，不再直接关闭；发起人可确认完结或退回处理中。
- 管理员可执行全部流程动作，但弹窗明确显示真实管理员账号，代办必须填写审计说明，活动日志不冒充发起人或验证人。
- 重大质量审批完成后仍进入“待发起人确认”；关闭历史保留，重新打开必须填写管理员审计说明。
- “提交验证准备度”与生命周期进度分离，避免 7 / 7 被误读为问题已经关闭。

### 内容、证据与完结库验收

- 详情按“问题总览、原因与方案、协同任务、决策记录、文件与证据、验证与关闭、全量日志”七个分支切换，核心主任务不再堆叠在同一长页面。
- 证据按“现场原始资料、原因分析资料、处理过程资料、验证证据、归档同步资料、其他未分类”六类展示；旧附件可在未关闭状态下重新归类。
- 已关闭问题进入“已完结问题库”，可继续按生产、计划、技术、工艺、质量、物料、设备及其他类型筛选。
- 关闭详情为只读状态：没有编辑、删除、上传入口，责任、验证和附件控件均不可修改；管理员需通过“重新打开”审计流程恢复处理。

### 迭代记录与可用性

1. 首轮验收发现选择“已关闭”后列表已切换，但详情仍可能保留之前的待确认问题，属于 P1 状态一致性风险；已改为筛选范围成为权威数据源，并自动选择该范围的第一条记录或清空详情。
2. 修正后重新验证已关闭、质量问题、设备问题和零结果筛选；列表、详情、计数和只读状态一致。
3. 发起人确认、管理员代办、文件分组、完结库筛选和重新打开弹窗均在 1366 × 1024 下完成实交互检查，没有主栏遮挡、下拉菜单越界、按钮裁切或页面级横向溢出。
4. 浏览器控制台应用 error / warning：0。

### 最终严重度检查

- P0：无。
- P1：无（首轮筛选一致性问题已修复并复验）。
- P2：无。
