...
这是 `v1.16.5.2-wecom-helper-pairing-state-fix` 的 Windows 10 / 11 便携式导入助手，使用 .NET 8 WPF 构建。它不会读取企业微信私有缓存、Cookie 或内部协议，而是接收用户主动拖出、复制或下载到本地的真实 PDF / 图片，再上传到网页中当前选定的工单和资料分类。
...
- 手动任务码只允许一个助手实例绑定；同一助手因双击或网络重试再次提交时幂等恢复当前任务，不会误报“任务码已使用”。任务 ticket 仍只保存在进程内存中。
...
- 协议唤起与手动任务码同时发生时由同一助手实例串行处理；连接成功后任务码输入框会自动清空，并立即显示客户、规格、分类和有效期。
